...
<!-- alef:hash:687606a041da9b7c4601d3e7c014724a9f1c605b7757ae5b8446099908231537 -->
...
| `minImageArea` | `int` | `xberg::CaptioningConfig::default_min_image_area()` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `batchSize` | `PlatformInt64` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `maxConcurrency` | `PlatformInt64` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `device` | `CandleDevicePreference?` | `CandleDevicePreference.Auto` | Optional per-call device override. |
...
| `ocrStrategy` | `OcrStrategy?` | `OcrStrategy.Auto` | Override the OCR page-selection strategy for this file. |
...
| `resultFormat` | `ResultFormat?` | `ResultFormat.Unified` | Override result format for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind?` | `GlmOcrTaskKind.Ocr` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference?` | `CandleDevicePreference.Auto` | Optional per-call device override. |
| `layoutMode` | `GlmOcrLayoutMode?` | `GlmOcrLayoutMode.WholePage` | Optional whole-page or layout-paired dispatch mode. |
...
| `rowSpan` | `int` | `default_span()` | Number of rows this cell spans. |
| `colSpan` | `int` | `default_span()` | Number of columns this cell spans. |
...
| `priority` | `int` | `default_priority()` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind?` | `PaddleOcrVlTaskKind.Ocr` | Optional per-call recognition task; the backend constructor's task is used when absent. |
...
| `caseSensitive` | `bool` | `default_case_sensitive()` | When `true`, match case-sensitively; otherwise prepend `(?i)` to the regex. |
...
| `caseSensitive` | `bool` | `default_case_sensitive()` | When `true`, match the value as-is; otherwise match ASCII-case-insensitively. |
...
| `maxContentSize` | `PlatformInt64` | `104857600` | Maximum string growth per document (100 MB) |
...
| `maxTableCells` | `PlatformInt64` | `100000` | Maximum cells per table (100,000) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant?` | `CandleTrocrVariant.BasePrinted` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference?` | `CandleDevicePreference.Auto` | Optional per-call device override. |
| `cacheDir` | `String?` | `null` | Optional Hugging Face cache root. |
| `hfRevision` | `String?` | `null` | Optional immutable Hugging Face model revision. |
