...
<!-- alef:hash:60b66495a61d5191933fcc227e0fa966dc9bf3c27c5797387279e15ae4a5f05b -->
...
| `MinImageArea` | `uint` | `xberg::CaptioningConfig::default_min_image_area()` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `BatchSize` | `ulong` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `MaxConcurrency` | `ulong` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `RowSpan` | `uint` | `default_span()` | Number of rows this cell spans. |
| `ColSpan` | `uint` | `default_span()` | Number of columns this cell spans. |
...
| `Priority` | `uint` | `default_priority()` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
...
| `MaxContentSize` | `ulong` | `104857600` | Maximum string growth per document (100 MB) |
| `MaxIterations` | `ulong` | `10000000` | Maximum iterations per operation |
| `MaxXmlDepth` | `ulong` | `1024` | Maximum XML depth (1024 levels) |
| `MaxTableCells` | `ulong` | `100000` | Maximum cells per table (100,000) |
