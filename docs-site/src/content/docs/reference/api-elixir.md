...
<!-- alef:hash:22d89aa2ba2ea5072378c4f37d4e2f2837cd6045ec47e0a468a3e984c1c080bb -->
...
| `min_image_area` | `integer()` | `xberg::CaptioningConfig::default_min_image_area()` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `batch_size` | `integer()` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `max_concurrency` | `integer()` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `device` | `CandleDevicePreference \| nil` | `:auto` | Optional per-call device override. |
...
| `ocr_strategy` | `OcrStrategy \| nil` | `:auto` | Override the OCR page-selection strategy for this file. |
...
| `result_format` | `ResultFormat \| nil` | `:unified` | Override result format for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind \| nil` | `:ocr` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference \| nil` | `:auto` | Optional per-call device override. |
| `layout_mode` | `GlmOcrLayoutMode \| nil` | `:whole_page` | Optional whole-page or layout-paired dispatch mode. |
...
| `row_span` | `integer()` | `default_span()` | Number of rows this cell spans. |
| `col_span` | `integer()` | `default_span()` | Number of columns this cell spans. |
...
| `priority` | `integer()` | `default_priority()` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind \| nil` | `:ocr` | Optional per-call recognition task; the backend constructor's task is used when absent. |
...
| `case_sensitive` | `boolean()` | `default_case_sensitive()` | When `true`, match case-sensitively; otherwise prepend `(?i)` to the regex. |
...
| `case_sensitive` | `boolean()` | `default_case_sensitive()` | When `true`, match the value as-is; otherwise match ASCII-case-insensitively. |
...
| `schema_name` | `String.t()` | `xberg::StructuredExtractionConfig::default_schema_name()` | Schema name passed to the LLM's structured output mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant \| nil` | `:base_printed` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference \| nil` | `:auto` | Optional per-call device override. |
| `cache_dir` | `String.t() \| nil` | `nil` | Optional Hugging Face cache root. |
| `hf_revision` | `String.t() \| nil` | `nil` | Optional immutable Hugging Face model revision. |
