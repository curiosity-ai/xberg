...
```elixir
@spec install_pdf_render_diagnostics() :: {:ok, term()} | {:error, term()}
def install_pdf_render_diagnostics()
```

**Example:**

```elixir
{:ok, result} = install_pdf_render_diagnostics()
```
...
```elixir
@spec take_pdf_oxide_render_warnings() :: {:ok, term()} | {:error, term()}
def take_pdf_oxide_render_warnings()
```

**Example:**

```elixir
{:ok, result} = take_pdf_oxide_render_warnings()
```
...
```elixir
@spec build_decoder_prompt_tokens(start_of_transcript, lang_id, transcribe, no_timestamps, timestamps) :: {:ok, term()} | {:error, term()}
def build_decoder_prompt_tokens(start_of_transcript, lang_id, transcribe, no_timestamps, timestamps)
```

**Example:**

```elixir
{:ok, result} = build_decoder_prompt_tokens(42, 42, 42, 42, true)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `start_of_transcript` | `integer()` | Yes | The start of transcript |
| `lang_id` | `integer()` | Yes | The lang id |
| `transcribe` | `integer()` | Yes | The transcribe |
| `no_timestamps` | `integer()` | Yes | The no timestamps |
...
```elixir
@spec timestamp_token_to_ms(token_id, timestamp_begin_id) :: {:ok, term()} | {:error, term()}
def timestamp_token_to_ms(token_id, timestamp_begin_id)
```

**Example:**

```elixir
{:ok, result} = timestamp_token_to_ms(42, 42)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `token_id` | `integer()` | Yes | The token id |
| `timestamp_begin_id` | `integer()` | Yes | The timestamp begin id |
...
| `breadcrumb_target` | `BreadcrumbTarget` | `:content` | **Deprecated and inert** (#1393): see `BreadcrumbTarget` for the full explanation. Neither variant has any effect on `content` any more — call `render_heading_breadcrumb` explicitly at index time instead. Kept only for backward compatibility. Default: `Content`. |
...
```elixir
{:ok, result} = DoctorCheck.pass("value", "value")
```
...
| `message` | `String.t()` | Yes | The message |
...
```elixir
{:ok, result} = DoctorCheck.warn("value", "value")
```
...
```elixir
{:ok, result} = DoctorCheck.fail("value", "value")
```
...
```elixir
{:ok, result} = DoctorCheck.skip("value", "value")
```
...
| `backend` | `String.t()` | — | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
| `language` | `list(String.t())` | `\[\]` | Language code(s) for OCR recognition. Defaults to `\["eng"\]`. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| `det_limit_side_len` | `integer()` | — | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `list(String.t())` | `\[\]` | Language code(s) for OCR recognition. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
returns `nil` for it (see `extraction/derive.rs`), so no renderer runs and
