...
```php
public static function takePdfOxideRenderWarnings(): array<ProcessingWarning>
```
...
```php
public static function buildDecoderPromptTokens(int $startOfTranscript, int $langId, int $transcribe, int $noTimestamps, bool $timestamps): array<int>
```
...
```php
public static function timestampTokenToMs(int $tokenId, int $timestampBeginId): int
```
