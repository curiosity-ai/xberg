...
<!-- alef:hash:a2bb9b003b83660f639ef86bcb582b7769eb2d3683c66abb6194a79c0756c8f9 -->
...
| `batchSize` | `long` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `maxConcurrency` | `long` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `qualityScore` | `@Nullable Double` | `null` | Document quality score from quality analysis. A value between 0.0 and 1.0 indicating the overall text quality. Previously stored in `metadata.additional["quality_score"]`. |
...
| `ocrStrategy` | `@Nullable OcrStrategy` | `OcrStrategy.AUTO` | Override the OCR page-selection strategy for this file. |
...
| `resultFormat` | `@Nullable ResultFormat` | `ResultFormat.UNIFIED` | Override result format for this file. |
| `outputFormat` | `@Nullable OutputFormat` | `OutputFormat.PLAIN` | Override output content format for this file. |
