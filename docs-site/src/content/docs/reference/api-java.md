...
```java
public static void ensureInitialized() throws XbergError
```
...
**Errors:** Throws `XbergErrorException`.
...
```java
public static void clearEmbeddingBackends() throws XbergError
```
...
```java
public static List<String> listEmbeddingBackends() throws XbergError
```
...
```java
public static List<String> listDocumentExtractors() throws XbergError
```
...
```java
public static void clearDocumentExtractors() throws XbergError
```
...
```java
public static List<String> listOcrBackends() throws XbergError
```
...
```java
public static void clearOcrBackends() throws XbergError
```
...
```java
public static List<String> listPostProcessors() throws XbergError
```
...
```java
public static void clearPostProcessors() throws XbergError
```
...
```java
public static List<String> listRenderers() throws XbergError
```
...
```java
public static void clearRenderers() throws XbergError
```
...
```java
public static void clearRerankerBackends() throws XbergError
```
...
```java
public static List<String> listRerankerBackends() throws XbergError
```
...
```java
public static void clearTokenizerBackends() throws XbergError
```
...
```java
public static List<String> listTokenizerBackends() throws XbergError
```
...
```java
public static List<String> listValidators() throws XbergError
```
...
```java
public static void clearValidators() throws XbergError
```
...
```java
public static void classifyChunks(ExtractedDocument result, ChunkClassificationConfig config) throws XbergError
```
...
```java
public static List<SparseEmbedding> embedSparseAsync(List<String> texts, SparseEmbeddingConfig config) throws XbergError
```
...
```java
public static List<MultiVectorEmbedding> embedMultiVectorAsync(List<String> texts, LateInteractionConfig config, boolean isQuery) throws XbergError
```
...
```java
public static boolean installPdfRenderDiagnostics()
```

**Example:**

```java
var result = installPdfRenderDiagnostics();
```
...
```java
var result = takePdfOxideRenderWarnings();
```

**Returns:** `List<ProcessingWarning>`
...
```java
public static List<Long> buildDecoderPromptTokens(int startOfTranscript, int langId, int transcribe, int noTimestamps, boolean timestamps)
```

**Example:**

```java
var result = buildDecoderPromptTokens(42, 42, 42, 42, true);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `startOfTranscript` | `int` | Yes | The start of transcript |
| `langId` | `int` | Yes | The lang id |
| `transcribe` | `int` | Yes | The transcribe |
| `noTimestamps` | `int` | Yes | The no timestamps |
| `timestamps` | `boolean` | Yes | The timestamps |
...
```java
public static int timestampTokenToMs(int tokenId, int timestampBeginId)
```

**Example:**

```java
var result = timestampTokenToMs(42, 42);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `tokenId` | `int` | Yes | The token id |
| `timestampBeginId` | `int` | Yes | The timestamp begin id |
...
| `accessKeyId` | `Optional<String>` | `null` | Explicit AWS access key ID. Secret — never logged. |
| `secretAccessKey` | `Optional<String>` | `null` | Explicit AWS secret access key. Secret — never logged. |
| `sessionToken` | `Optional<String>` | `null` | Explicit AWS session token for temporary credentials. Secret — never logged. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `delimiter` | `Optional<String>` | `null` | Field delimiter, as a single-character string (e.g. `","`, `";"`, `"\t"`, `"\|"`). When `null` (default), the delimiter is auto-detected from a sample of the file. Must be exactly one ASCII byte when set — `ExtractionConfig.validate` rejects an empty string or a multi-byte value with a helpful error. The TSV MIME type (`text/tab-separated-values`) always forces `\t` regardless of this setting. |
...
```java
var result = DoctorCheck.pass("value", "value");
```
...
```java
var result = DoctorCheck.warn("value", "value");
```
...
```java
var result = DoctorCheck.fail("value", "value");
```
...
```java
var result = DoctorCheck.skip("value", "value");
```
...
```java
public ExtractedDocument extract(ExtractInput input, ExtractionConfig config) throws XbergError
```
...
```java
public List<List<Float>> embed(List<String> texts) throws XbergError
```
...
| `detectedLanguageConfidences` | `Optional<List<LanguageConfidence>>` | `Collections.emptyList()` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `null` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `csv` | `Optional<CsvConfig>` | `null` | CSV/TSV extraction configuration (None = use defaults). Lets callers set an explicit delimiter and declare comment-line prefixes to skip, instead of relying solely on delimiter auto-detection. See `CsvConfig` for details. |
...
```java
public void validate() throws XbergError
```
...
| `providers` | `Optional<List<LlmProviderConfig>>` | `Collections.emptyList()` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
```java
public ExtractedDocument processImage(byte[] imageBytes, OcrConfig config) throws XbergError
```
...
```java
public ExtractedDocument processImageFile(String path, OcrConfig config) throws XbergError
```
...
```java
public ExtractedDocument processDocument(String path, OcrConfig config) throws XbergError
```
...
| `detLimitSideLen` | `int` | — | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
...
| `author` | `Optional<String>` | `/* serde(default) */` | Author/creator of the annotation (PDF `/T` entry). |
| `modified` | `Optional<String>` | `/* serde(default) */` | Last modification date of the annotation (PDF `/M` entry), as a raw PDF date string (e.g. `"D:20240115120000Z"`). |
| `color` | `Optional<String>` | `/* serde(default) */` | Annotation colour (PDF `/C` entry), normalised to a CSS-compatible `#rrggbb` hex string. Gray and CMYK colour spaces are converted to RGB. |
| `subject` | `Optional<String>` | `/* serde(default) */` | Subject of the annotation (PDF `/Subj` entry). |
| `quadPoints` | `Optional<List<BoundingBox>>` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
| `markedText` | `Optional<String>` | `/* serde(default) */` | The document text covered by `Self.quad_points`, recovered from the page content underneath the marked-up region. Populated for Highlight, Underline, StrikeOut, and Squiggly annotations when the underlying text could be recovered. |
...
| `layoutGatedPages` | `Optional<List<Integer>>` | `Collections.emptyList()` | Pages the `auto` layout strategy skipped (1-indexed). `null` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
| `layoutGateReasons` | `Optional<List<String>>` | `Collections.emptyList()` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `null` unless layout detection ran with `LayoutStrategy.Auto`. |
...
```java
public void initialize() throws XbergError
```
...
```java
public void shutdown() throws XbergError
```
...
```java
public void process(ExtractedDocument result, ExtractionConfig config) throws XbergError
```
...
```java
public String renderResult(ExtractedDocument result) throws XbergError
```
...
```java
public List<Float> rerank(String query, List<String> documents) throws XbergError
```
...
| `value` | `Optional<Object>` | `null` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `null` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `List<String>` | `Collections.emptyList()` | Language code(s) for OCR recognition. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
```java
public void validate(ExtractedDocument result, ExtractionConfig config) throws XbergError
```
