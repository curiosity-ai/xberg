...
<!-- alef:hash:9c785a453e458089d6f029bb9eefdc4ba6fa2307592d68819a9dc23b82f1dd11 -->
...
| `minImageArea` | `UInt32` | `xberg::CaptioningConfig::default_min_image_area()` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `batchSize` | `UInt` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `maxConcurrency` | `UInt` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `rowSpan` | `UInt32` | `default_span()` | Number of rows this cell spans. |
| `colSpan` | `UInt32` | `default_span()` | Number of columns this cell spans. |
...
| `priority` | `UInt32` | `default_priority()` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
...
| `maxContentSize` | `UInt` | `104857600` | Maximum string growth per document (100 MB) |
| `maxIterations` | `UInt` | `10000000` | Maximum iterations per operation |
| `maxXmlDepth` | `UInt` | `1024` | Maximum XML depth (1024 levels) |
| `maxTableCells` | `UInt` | `100000` | Maximum cells per table (100,000) |
