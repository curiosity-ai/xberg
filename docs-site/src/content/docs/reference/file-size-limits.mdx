...
### 4. Programmatic Configuration (Rust only)

`ApiSizeLimits` and `create_router_with_limits` are part of the core `xberg` Rust
crate's `api` module, used when embedding the Xberg Axum router directly. They are
not exposed by any language binding (Python, TypeScript, Go, Java, C#, Ruby,
etc.) — those consumers configure limits through the `XBERG_MAX_REQUEST_BODY_BYTES`
/ `XBERG_MAX_MULTIPART_FIELD_BYTES` environment variables or a config file when
running the `xberg serve` binary, as shown above.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // `ApiSizeLimits` only exposes public fields; build it directly rather
    // than via a constructor method.
...
- [Configuration Guide](/guides/configuration/) - Extraction configuration options
- [API Server Guide](/guides/api-server/) - Complete API server documentation
- [Docker Deployment](/guides/docker/) - Docker setup and configuration
- [Performance Tuning](/guides/api-server/) - Advanced performance optimization
