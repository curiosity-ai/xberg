...
```typescript
function installPdfRenderDiagnostics(): boolean
```
...
```typescript
function buildDecoderPromptTokens(startOfTranscript: number, langId: number, transcribe: number, noTimestamps: number, timestamps: boolean): Array<number>
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `startOfTranscript` | `number` | Yes | The start of transcript |
| `langId` | `number` | Yes | The lang id |
| `transcribe` | `number` | Yes | The transcribe |
| `noTimestamps` | `number` | Yes | The no timestamps |
...
```typescript
function timestampTokenToMs(tokenId: number, timestampBeginId: number): number
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `tokenId` | `number` | Yes | The token id |
| `timestampBeginId` | `number` | Yes | The timestamp begin id |
...
```typescript
static pass(name: string, message: string): DoctorCheck
```
...
```typescript
static warn(name: string, message: string): DoctorCheck
```
...
```typescript
static fail(name: string, message: string): DoctorCheck
```
...
```typescript
static skip(name: string, message: string): DoctorCheck
```
...
| `detLimitSideLen` | `number` | — | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
