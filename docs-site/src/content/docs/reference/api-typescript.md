...
<!-- alef:hash:82af22949d0b60364baf99fb281a715397e2014c3e542a7db9b10b7f0711816d -->
...
| `minImageArea` | `number` | `xberg::CaptioningConfig::default_min_image_area()` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `batchSize` | `number` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `maxConcurrency` | `number` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `device` | `CandleDevicePreference \| null` | `CandleDevicePreference.Auto` | Optional per-call device override. |
...
| `ocrStrategy` | `OcrStrategy \| null` | `OcrStrategy.Auto` | Override the OCR page-selection strategy for this file. |
...
| `resultFormat` | `ResultFormat \| null` | `ResultFormat.Unified` | Override result format for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind \| null` | `GlmOcrTaskKind.Ocr` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference \| null` | `CandleDevicePreference.Auto` | Optional per-call device override. |
| `layoutMode` | `GlmOcrLayoutMode \| null` | `GlmOcrLayoutMode.WholePage` | Optional whole-page or layout-paired dispatch mode. |
...
| `rowSpan` | `number` | `default_span()` | Number of rows this cell spans. |
| `colSpan` | `number` | `default_span()` | Number of columns this cell spans. |
...
| `priority` | `number` | `default_priority()` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind \| null` | `PaddleOcrVlTaskKind.Ocr` | Optional per-call recognition task; the backend constructor's task is used when absent. |
...
| `caseSensitive` | `boolean` | `default_case_sensitive()` | When `true`, match case-sensitively; otherwise prepend `(?i)` to the regex. |
...
| `caseSensitive` | `boolean` | `default_case_sensitive()` | When `true`, match the value as-is; otherwise match ASCII-case-insensitively. |
...
| `schemaName` | `string` | `xberg::StructuredExtractionConfig::default_schema_name()` | Schema name passed to the LLM's structured output mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant \| null` | `CandleTrocrVariant.BasePrinted` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference \| null` | `CandleDevicePreference.Auto` | Optional per-call device override. |
| `cacheDir` | `string \| null` | `null` | Optional Hugging Face cache root. |
| `hfRevision` | `string \| null` | `null` | Optional immutable Hugging Face model revision. |
