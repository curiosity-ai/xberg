...
<!-- alef:hash:c1e1f3f5eea122a4c5250fd18afc231e59c650a68ffbc7549eb008b97516a0ae -->
...
| `min_image_area` | `int` | `xberg::CaptioningConfig::default_min_image_area()` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `max_concurrency` | `int` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `device` | `CandleDevicePreference \| None` | `CandleDevicePreference.AUTO` | Optional per-call device override. |
...
| `ocr_strategy` | `OcrStrategy \| None` | `OcrStrategy.AUTO` | Override the OCR page-selection strategy for this file. |
...
| `result_format` | `ResultFormat \| None` | `ResultFormat.UNIFIED` | Override result format for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `GlmOcrTaskKind \| None` | `GlmOcrTaskKind.OCR` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `CandleDevicePreference \| None` | `CandleDevicePreference.AUTO` | Optional per-call device override. |
| `layout_mode` | `GlmOcrLayoutMode \| None` | `GlmOcrLayoutMode.WHOLE_PAGE` | Optional whole-page or layout-paired dispatch mode. |
...
| `row_span` | `int` | `default_span()` | Number of rows this cell spans. |
| `col_span` | `int` | `default_span()` | Number of columns this cell spans. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `PaddleOcrVlTaskKind \| None` | `PaddleOcrVlTaskKind.OCR` | Optional per-call recognition task; the backend constructor's task is used when absent. |
...
| `schema_name` | `str` | `xberg::StructuredExtractionConfig::default_schema_name()` | Schema name passed to the LLM's structured output mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `CandleTrocrVariant \| None` | `CandleTrocrVariant.BASE_PRINTED` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `CandleDevicePreference \| None` | `CandleDevicePreference.AUTO` | Optional per-call device override. |
| `cache_dir` | `str \| None` | `None` | Optional Hugging Face cache root. |
| `hf_revision` | `str \| None` | `None` | Optional immutable Hugging Face model revision. |
