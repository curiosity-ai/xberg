...
```python
result = install_pdf_render_diagnostics()
```
...
```python
result = take_pdf_oxide_render_warnings()
```
...
```python
def build_decoder_prompt_tokens(start_of_transcript: int, lang_id: int, transcribe: int, no_timestamps: int, timestamps: bool) -> list[int]
```

**Example:**

```python
result = build_decoder_prompt_tokens(42, 42, 42, 42, True)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `start_of_transcript` | `int` | Yes | The start of transcript |
| `lang_id` | `int` | Yes | The lang id |
| `transcribe` | `int` | Yes | The transcribe |
| `no_timestamps` | `int` | Yes | The no timestamps |
...
```python
result = timestamp_token_to_ms(42, 42)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `token_id` | `int` | Yes | The token id |
| `timestamp_begin_id` | `int` | Yes | The timestamp begin id |
...
```python
@staticmethod
def pass(name: str, message: str) -> DoctorCheck
```
...
| `backend` | `str` | — | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
| `language` | `list\[str\]` | `\[\]` | Language code(s) for OCR recognition. Defaults to `\["eng"\]`. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| `det_limit_side_len` | `int` | — | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `list\[str\]` | `\[\]` | Language code(s) for OCR recognition. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
