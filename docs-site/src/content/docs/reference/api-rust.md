...
```rust
pub fn ensure_initialized() -> Result<(), XbergError>
```
...
```rust
pub async fn classify_chunks(result: ExtractedDocument, config: ChunkClassificationConfig) -> Result<(), XbergError>
```
...
```rust
let result = install_pdf_render_diagnostics();
```
...
```rust
let result = take_pdf_oxide_render_warnings();
```

**Returns:** `Vec<ProcessingWarning>`
...
```rust
pub fn build_decoder_prompt_tokens(start_of_transcript: u32, lang_id: u32, transcribe: u32, no_timestamps: u32, timestamps: bool) -> Vec<i64>
```

**Example:**

```rust
let result = build_decoder_prompt_tokens(42, 42, 42, 42, true);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `start_of_transcript` | `u32` | Yes | The start of transcript |
| `lang_id` | `u32` | Yes | The lang id |
| `transcribe` | `u32` | Yes | The transcribe |
| `no_timestamps` | `u32` | Yes | The no timestamps |
...
```rust
let result = timestamp_token_to_ms(42, 42);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `token_id` | `u32` | Yes | The token id |
| `timestamp_begin_id` | `u32` | Yes | The timestamp begin id |
...
| `breadcrumb_target` | `BreadcrumbTarget` | `BreadcrumbTarget::Content` | **Deprecated and inert** (#1393): see `BreadcrumbTarget` for the full explanation. Neither variant has any effect on `content` any more — call `render_heading_breadcrumb` explicitly at index time instead. Kept only for backward compatibility. Default: `Content`. |
...
| `output_format` | `String` | — | Target output format (Markdown, plain text, etc.). |
...
```rust
pub fn pass(name: &str, message: &str) -> DoctorCheck
```
...
```rust
pub fn warn(name: &str, message: &str) -> DoctorCheck
```
...
```rust
pub fn fail(name: &str, message: &str) -> DoctorCheck
```
...
```rust
pub fn skip(name: &str, message: &str) -> DoctorCheck
```
...
| `det_limit_side_len` | `u32` | — | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
...
```rust
pub async fn process(&self, result: ExtractedDocument, config: ExtractionConfig) -> Result<(), XbergError>
```
...
```rust
pub async fn rerank(&self, query: &str, documents: Vec<String>) -> Result<Vec<f32>, XbergError>
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `Vec<String>` | `vec!\[\]` | Language code(s) for OCR recognition. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
