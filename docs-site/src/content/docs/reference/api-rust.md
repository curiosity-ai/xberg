...
<!-- alef:hash:046f6a7f032ee43b42728df1c132d9c51449ee6d9faf20aac9913df729e2875c -->
...
| `batch_size` | `usize` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `max_concurrency` | `usize` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `device` | `Option<CandleDevicePreference>` | `CandleDevicePreference::Auto` | Optional per-call device override. |
...
| `ocr_strategy` | `Option<OcrStrategy>` | `OcrStrategy::Auto` | Override the OCR page-selection strategy for this file. |
...
| `result_format` | `Option<ResultFormat>` | `ResultFormat::Unified` | Override result format for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `Option<GlmOcrTaskKind>` | `GlmOcrTaskKind::Ocr` | Optional recognition task; the backend constructor's task is used when absent. |
| `device` | `Option<CandleDevicePreference>` | `CandleDevicePreference::Auto` | Optional per-call device override. |
| `layout_mode` | `Option<GlmOcrLayoutMode>` | `GlmOcrLayoutMode::WholePage` | Optional whole-page or layout-paired dispatch mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `Option<PaddleOcrVlTaskKind>` | `PaddleOcrVlTaskKind::Ocr` | Optional per-call recognition task; the backend constructor's task is used when absent. |
...
| `schema_name` | `String` | `xberg::StructuredExtractionConfig::default_schema_name()` | Schema name passed to the LLM's structured output mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `variant` | `Option<CandleTrocrVariant>` | `CandleTrocrVariant::BasePrinted` | Optional model variant; the backend constructor's variant is used when absent. |
| `device` | `Option<CandleDevicePreference>` | `CandleDevicePreference::Auto` | Optional per-call device override. |
| `cache_dir` | `Option<String>` | `Default::default()` | Optional Hugging Face cache root. |
| `hf_revision` | `Option<String>` | `Default::default()` | Optional immutable Hugging Face model revision. |
