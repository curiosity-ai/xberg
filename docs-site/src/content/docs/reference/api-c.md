...
```c
XbergDoctorReport* xberg_doctor(XbergExtractionConfig config);
```

**Example:**

```c
XbergDoctorReport *result = xberg_doctor(NULL);
```
...
```c
bool xberg_install_pdf_render_diagnostics();
```

**Example:**

```c
bool result = xberg_install_pdf_render_diagnostics();
```
...
```c
int64_t* xberg_build_decoder_prompt_tokens(uint32_t start_of_transcript, uint32_t lang_id, uint32_t transcribe, uint32_t no_timestamps, bool timestamps);
```

**Example:**

```c
int64_t* result = xberg_build_decoder_prompt_tokens(42, 42, 42, 42, true);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `start_of_transcript` | `uint32_t` | Yes | The start of transcript |
| `lang_id` | `uint32_t` | Yes | The lang id |
| `transcribe` | `uint32_t` | Yes | The transcribe |
| `no_timestamps` | `uint32_t` | Yes | The no timestamps |
...
```c
uint32_t xberg_timestamp_token_to_ms(uint32_t token_id, uint32_t timestamp_begin_id);
```

**Example:**

```c
uint32_t result = xberg_timestamp_token_to_ms(42, 42);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `token_id` | `uint32_t` | Yes | The token id |
| `timestamp_begin_id` | `uint32_t` | Yes | The timestamp begin id |
...
| `breadcrumb_target` | `XbergBreadcrumbTarget` | `XBERG_XBERG_CONTENT` | **Deprecated and inert** (#1393): see `BreadcrumbTarget` for the full explanation. Neither variant has any effect on `content` any more — call `render_heading_breadcrumb` explicitly at index time instead. Kept only for backward compatibility. Default: `Content`. |
...
| `status` | `XbergProbeStatus` | — | Pass / warn / fail / skip verdict. |
...
```c
XbergDoctorCheck xberg_pass(const char* name, const char* message);
```

**Example:**

```c
XbergDoctorCheck *result = xberg_pass("value", "value");
```
...
| `message` | `const char*` | Yes | The message |

**Returns:** `XbergDoctorCheck`
...
```c
XbergDoctorCheck xberg_warn(const char* name, const char* message);
```

**Example:**

```c
XbergDoctorCheck *result = xberg_warn("value", "value");
```
...
```c
XbergDoctorCheck xberg_fail(const char* name, const char* message);
```

**Example:**

```c
XbergDoctorCheck *result = xberg_fail("value", "value");
```
...
```c
XbergDoctorCheck xberg_skip(const char* name, const char* message);
```

**Example:**

```c
XbergDoctorCheck *result = xberg_skip("value", "value");
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `checks` | `XbergDoctorCheck*` | `NULL` | Individual check verdicts, in execution order. |
...
```c
bool result = xberg_is_ok(instance);
```
...
| `detected_language_confidences` | `XbergLanguageConfidence**` | `NULL` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `NULL` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `csv` | `XbergCsvConfig*` | `NULL` | CSV/TSV extraction configuration (None = use defaults). Lets callers set an explicit delimiter and declare comment-line prefixes to skip, instead of relying solely on delimiter auto-detection. See `CsvConfig` for details. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `strategy` | `XbergLayoutStrategy` | `XBERG_XBERG_ALWAYS` | Which pages the layout model runs on. Defaults to `LayoutStrategy.Always`, the historical behavior: every page is rendered and inferred. `LayoutStrategy.Auto` pre-screens pages with cheap signals and skips the model where it cannot help. |
...
| `model_limits` | `void**` | `NULL` | Per-model spend limits in USD, keyed by model name. |
...
| `backend_config` | `void**` | `NULL` | Backend-specific configuration key/value pairs. |
...
| `providers` | `XbergLlmProviderConfig**` | `NULL` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
| `cache` | `XbergLlmCacheConfig*` | `NULL` | Response cache configuration. Mirrors liter-llm's `LlmConfig.cache`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. Boxed for the same reason as `bedrock` (a4579589ac): `LlmConfig` is the payload of `EmbeddingModelType.Llm` and `RerankerModelType.Llm`, whose other variants are tens of bytes. Inlining this and the two sub-configs below pushed that variant to 480 bytes and tripped `clippy.large_enum_variant` on the `--features full` leg. ~keep |
| `budget` | `XbergLlmBudgetConfig*` | `NULL` | Budget enforcement configuration. Mirrors liter-llm's `LlmConfig.budget`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `rate_limit` | `XbergLlmRateLimitConfig*` | `NULL` | Per-model rate limiting configuration. Mirrors liter-llm's `LlmConfig.rate_limit`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
| `backend` | `const char*` | — | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
| `language` | `const char**` | `NULL` | Language code(s) for OCR recognition. Defaults to `\["eng"\]`. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| `det_limit_side_len` | `uint32_t` | — | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
...
| `quad_points` | `XbergBoundingBox**` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
...
| `layout_gated_pages` | `uint32_t**` | `NULL` | Pages the `auto` layout strategy skipped (1-indexed). `NULL` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
...
| `value` | `void**` | `NULL` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `NULL` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `const char**` | `NULL` | Language code(s) for OCR recognition. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| Value | Description |
|-------|-------------|
| `XBERG_CONTENT` | Inert (#1393). Previously prepended the heading breadcrumb into chunk `content`; no longer has any effect — `content` is left untouched, exactly like `Metadata`. Kept as the default only for wire/API compatibility. |
| `XBERG_METADATA` | Inert (#1393), and was already a no-op on `content` before this change. Kept only for backward compatibility, since `Content` is no longer distinguishable from it. |
...
| `XBERG_STRUCTURED` | Metadata-only label; content is identical to `OutputFormat.Plain`. No dedicated renderer exists yet, so this attaches no OCR element metadata. See the enum-level docs above. |
