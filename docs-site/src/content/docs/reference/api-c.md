...
<!-- alef:hash:7ea8a506ce47159e1b1005ae0b5990875123621e3f3bb69794f802de2fa3f759 -->
...
|-------|------|---------|-------------|
| `provider` | `XBERGAlefHandle` | `XBERG_XBERG_AUTO` | Execution provider to use for ONNX inference. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `XBERGAlefHandle` | `XBERG_XBERG_AUTO` | When to use the headless browser fallback. |
| `backend` | `XBERGAlefHandle` | `XBERG_XBERG_CHROMIUMOXIDE` | Browser backend used to render JavaScript-heavy pages. |
...
| `wait` | `XBERGAlefHandle` | `XBERG_XBERG_NETWORK_IDLE` | Wait strategy after browser navigation. |
...
| `min_image_area` | `uint32_t` | `xberg::CaptioningConfig::default_min_image_area()` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `batch_size` | `uintptr_t` | `xberg::ChunkClassificationConfig::default_batch_size()` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `max_concurrency` | `uintptr_t` | `xberg::ChunkClassificationConfig::default_max_concurrency()` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `chunker_type` | `XBERGAlefHandle` | `XBERG_XBERG_TEXT` | Type of chunker to use (Text or Markdown). Default: Text |
...
| `sizing` | `XBERGAlefHandle` | `XBERG_XBERG_CHARACTERS` | How to measure chunk size. Default: `Characters` (Unicode character count). Enable `chunking-tiktoken` or `chunking-tokenizers` features for token-based sizing. |
| `topic_threshold` | `float*` | `NULL` | Optional cosine similarity threshold for semantic topic boundary detection. Only used when `chunker_type` is `Semantic` and an `EmbeddingConfig` is provided. You almost never need to set this. When omitted, defaults to `0.75` which works well for most documents. Lower values detect more topic boundaries (more, smaller chunks); higher values detect fewer. Range: `0.0..=1.0`. |
| `table_chunking` | `XBERGAlefHandle` | `XBERG_XBERG_SPLIT` | How to handle markdown tables that exceed the chunk size limit. Only applies when `chunker_type` is `Markdown`. - `Split` (default) — tables are split at row boundaries; continuation chunks do not repeat the header. - `RepeatHeader` — the table header row and separator are prepended to every continuation chunk so each chunk is self-contained. Default: `Split` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `heading_style` | `XBERGAlefHandle` | `XBERG_XBERG_ATX` | Heading style to use in Markdown output (ATX `#` or Setext underline). |
| `list_indent_type` | `XBERGAlefHandle` | `XBERG_XBERG_SPACES` | How to indent nested list items (spaces or tab). |
...
| `highlight_style` | `XBERGAlefHandle` | `XBERG_XBERG_DOUBLE_EQUAL` | Style used for `<mark>` / highlighted text (e.g. `==text==`). |
...
| `newline_style` | `XBERGAlefHandle` | `XBERG_XBERG_SPACES` | How to encode hard line breaks (`<br>`) in Markdown. |
| `code_block_style` | `XBERGAlefHandle` | `XBERG_XBERG_BACKTICKS` | Style used for fenced code blocks (backticks or tilde). |
...
| `url_escape_style` | `XBERGAlefHandle` | `XBERG_XBERG_ANGLE` | URL encoding strategy for link and image destinations. Controls how special characters in URL destinations are escaped: - `UrlEscapeStyle.Angle` (default) — wraps the destination in angle brackets when it contains spaces or newlines. Some parsers misinterpret `>` inside such a destination. - `UrlEscapeStyle.Percent` — percent-encodes every character that is not an RFC 3986 unreserved character or `/`, producing a destination that all Markdown parsers handle correctly even when the URL contains `<`, `>`, spaces, or parentheses. |
| `link_style` | `XBERGAlefHandle` | `XBERG_XBERG_INLINE` | Link rendering style (inline or reference). |
| `output_format` | `XBERGAlefHandle` | `XBERG_XBERG_MARKDOWN` | Target output format (Markdown, plain text, etc.). |
...
| `tier_strategy` | `XBERGAlefHandle` | `XBERG_XBERG_AUTO` | Which conversion tier to use. - `TierStrategy.Auto` (default) — automatically choose the best path. - `TierStrategy.Tier2` — always use the Tier-2 DOM-walk path. - `TierStrategy.Tier1` — always attempt Tier-1 (testkit only). |
...
| `crawl_strategy` | `XBERGAlefHandle` | `XBERG_XBERG_BFS` | Traversal order. Defaults to breadth-first. A frontier or strategy set explicitly on `CrawlEngineBuilder` takes precedence over this field. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `kind` | `XBERGAlefHandle` | `XBERG_XBERG_URI` | Source kind. `bytes` requires `bytes`; `uri` requires `uri`. |
...
| `ocr_strategy` | `XBERGAlefHandle` | `XBERG_XBERG_AUTO` | Which pages get OCR'd when neither `force_ocr` nor `force_ocr_pages` applies. Defaults to `OcrStrategy.Auto`, which OCRs only pages whose native text fails a quality check. Only applies to PDF documents. Cannot be `OcrStrategy.ScannedPages` while `disable_ocr` is `true`. |
...
| `result_format` | `XBERGAlefHandle` | `XBERG_XBERG_UNIFIED` | Result structure format Controls whether results are returned in unified format (default) with all content in the `content` field, or element-based format with semantic elements (for Unstructured-compatible output). |
...
| `jupyter_cell_rendering` | `XBERGAlefHandle` | `XBERG_XBERG_BOTH` | Controls how Jupyter notebook (`.ipynb`) code cells are rendered. - `Both` (default): code source plus the notebook's saved outputs - `Source`: only the code source (fenced code blocks) - `Outputs`: only the saved outputs Cells are never executed; `Outputs`/`Both` surface only outputs already stored in the notebook. |
...
| `ocr_strategy` | `XBERGAlefHandle` | `XBERG_XBERG_AUTO` | Override the OCR page-selection strategy for this file. |
...
| `result_format` | `XBERGAlefHandle` | `XBERG_XBERG_UNIFIED` | Override result format for this file. |
| `output_format` | `XBERGAlefHandle` | `XBERG_XBERG_PLAIN` | Override output content format for this file. |
...
| `row_span` | `uint32_t` | `default_span()` | Number of rows this cell spans. |
| `col_span` | `uint32_t` | `default_span()` | Number of columns this cell spans. |
...
| `theme` | `XBERGAlefHandle` | `XBERG_XBERG_UNSTYLED` | Built-in colour/typography theme. Default: `HtmlTheme.Unstyled`. |
...
| `deskew` | `int32_t` | `true` | Correct skew (tilted images). |
| `denoise` | `int32_t` | `false` | Remove noise from the image. |
| `contrast_enhance` | `int32_t` | `false` | Enhance contrast for better text visibility. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `algorithm` | `XBERGAlefHandle` | `XBERG_XBERG_YAKE` | Algorithm to use for extraction. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `strategy` | `XBERGAlefHandle` | `XBERG_XBERG_ALWAYS` | Which pages the layout model runs on. Defaults to `LayoutStrategy.Always`, the historical behavior: every page is rendered and inferred. `LayoutStrategy.Auto` pre-screens pages with cheap signals and skips the model where it cannot help. |
...
| `table_model` | `XBERGAlefHandle` | `XBERG_XBERG_TATR` | Table structure recognition model. Controls which model is used for table cell detection within layout-detected table regions. Defaults to `TableModel.Tatr`. |
...
| `table_overlap_preference` | `XBERGAlefHandle` | `XBERG_XBERG_CONTENT` | How to resolve overlapping native vs layout tables. When a native table and a layout (TATR/SLANeXT) table overlap on the same region, this controls which one is kept. Defaults to `TableOverlapPreference.Content` (historical behavior: keep the table with more content). Set to `TableOverlapPreference.Native` to favor source reading order (higher text F1) over the model's cell reflow. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend` | `XBERGAlefHandle` | `XBERG_XBERG_ONNX` | Backend that runs the entity detection. |
...
| `geometry` | `XBERGAlefHandle` | `XBERG_XBERG_RECTANGLE` | Bounding geometry (rectangle or quadrilateral). |
| `confidence` | `XBERGAlefHandle` | — | Confidence scores for detection and recognition. |
| `level` | `XBERGAlefHandle` | `XBERG_XBERG_LINE` | Hierarchical level (word, line, block, page). |
...
| `min_level` | `XBERGAlefHandle` | `XBERG_XBERG_LINE` | Minimum hierarchical level to include. Elements below this level (e.g., words when min_level is Line) will be excluded. |
...
| `priority` | `uint32_t` | `default_priority()` | Priority weight (higher = tried first). Stages are sorted by priority descending. |
...
| `backend` | `XBERGAlefHandle` | `XBERG_XBERG_NATIVE` | Which engine parses and renders this PDF. Defaults to `PdfBackend.Native`. Selecting `PdfBackend.Pdfium` requires the `pdf-pdfium` feature and is rejected otherwise; the pdfium engine is also deliberately narrower in scope than `Native` -- see `PdfBackend` and `extractors.pdf.pdfium_engine` for details. |
...
| `preset` | `XBERGAlefHandle` | `XBERG_XBERG_STANDARD` | Preprocessing preset level (Minimal, Standard, Aggressive) |
...
| `strategy` | `XBERGAlefHandle` | `XBERG_XBERG_MASK` | Strategy applied to every match. |
...
| `case_sensitive` | `int32_t` | `default_case_sensitive()` | When `true`, match case-sensitively; otherwise prepend `(?i)` to the regex. |
...
| `case_sensitive` | `int32_t` | `default_case_sensitive()` | When `true`, match the value as-is; otherwise match ASCII-case-insensitively. |
...
| `schema_name` | `const char*` | `xberg::StructuredExtractionConfig::default_schema_name()` | Schema name passed to the LLM's structured output mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `strategy` | `XBERGAlefHandle` | `XBERG_XBERG_EXTRACTIVE` | Summarisation strategy. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `level` | `XBERGAlefHandle` | `XBERG_XBERG_MODERATE` | Reduction intensity level. |
...
| `model` | `XBERGAlefHandle` | `XBERG_XBERG_TINY` | Whisper model size to use. Smaller = faster + lower memory. `tiny` is the pragmatic default for first-time users and CI. |
...
| `content_mode` | `XBERGAlefHandle` | `XBERG_XBERG_CHUNKS` | Content rendering mode for code extraction. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `XBERGAlefHandle` | `XBERG_XBERG_AUTO` | URL extraction mode. |
