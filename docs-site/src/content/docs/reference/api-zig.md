...
```zig
pub fn list_embedding_backends() XbergError![]const [:0]const u8
```
...
```zig
pub fn unregister_document_extractor(name: [:0]const u8) XbergError!void
```
...
```zig
pub fn list_document_extractors() XbergError![]const [:0]const u8
```
...
```zig
pub fn list_ocr_backends() XbergError![]const [:0]const u8
```
...
```zig
pub fn list_post_processors() XbergError![]const [:0]const u8
```
...
```zig
pub fn unregister_renderer(name: [:0]const u8) XbergError!void
```
...
```zig
pub fn list_renderers() XbergError![]const [:0]const u8
```
...
```zig
pub fn list_reranker_backends() XbergError![]const [:0]const u8
```
...
```zig
pub fn list_tokenizer_backends() XbergError![]const [:0]const u8
```
...
```zig
pub fn list_validators() XbergError![]const [:0]const u8
```
...
```zig
pub fn classify_chunks(result: ExtractedDocument, config: ChunkClassificationConfig) XbergError!void
```
...
```zig
const result = installPdfRenderDiagnostics();
```
...
```zig
pub fn take_pdf_oxide_render_warnings() []const ProcessingWarning
```
...
**Returns:** `[]const ProcessingWarning`

---

#### buildDecoderPromptTokens()
...
```zig
pub fn build_decoder_prompt_tokens(start_of_transcript: u32, lang_id: u32, transcribe: u32, no_timestamps: u32, timestamps: bool) []const i64
```

**Example:**

```zig
const result = buildDecoderPromptTokens(42, 42, 42, 42, true);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `startOfTranscript` | `u32` | Yes | The start of transcript |
| `langId` | `u32` | Yes | The lang id |
| `transcribe` | `u32` | Yes | The transcribe |
| `noTimestamps` | `u32` | Yes | The no timestamps |
...
```zig
const result = timestampTokenToMs(42, 42);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `tokenId` | `u32` | Yes | The token id |
| `timestampBeginId` | `u32` | Yes | The timestamp begin id |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `region` | `\[:0\]const u8?` | `null` | AWS region (e.g. `"us-east-1"`). |
| `crossRegionPrefix` | `\[:0\]const u8?` | `null` | Cross-region inference profile prefix (e.g. `"us"`). |
| `accessKeyId` | `\[:0\]const u8?` | `null` | Explicit AWS access key ID. Secret — never logged. |
| `secretAccessKey` | `\[:0\]const u8?` | `null` | Explicit AWS secret access key. Secret — never logged. |
| `sessionToken` | `\[:0\]const u8?` | `null` | Explicit AWS session token for temporary credentials. Secret — never logged. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[:0\]const u8` | — | Check identifier, e.g. `ocr.tesseract` or `layout.rtdetr`. |
| `status` | `ProbeStatus` | — | Pass / warn / fail / skip verdict. |
| `message` | `\[:0\]const u8` | — | One-line reason or detail (e.g. missing language, resolved path, error). |
...
```zig
pub fn pass(name: [:0]const u8, message: [:0]const u8) DoctorCheck
```

**Example:**

```zig
const result = DoctorCheck.pass("value", "value");
```
...
|------|------|----------|-------------|
| `name` | `\[:0\]const u8` | Yes | The name |
| `message` | `\[:0\]const u8` | Yes | The message |
...
```zig
pub fn warn(name: [:0]const u8, message: [:0]const u8) DoctorCheck
```

**Example:**

```zig
const result = DoctorCheck.warn("value", "value");
```
...
```zig
pub fn fail(name: [:0]const u8, message: [:0]const u8) DoctorCheck
```

**Example:**

```zig
const result = DoctorCheck.fail("value", "value");
```
...
```zig
pub fn skip(name: [:0]const u8, message: [:0]const u8) DoctorCheck
```

**Example:**

```zig
const result = DoctorCheck.skip("value", "value");
```
...
```zig
pub fn isOk(self: *const DoctorReport) bool
```

**Example:**

```zig
const result = instance.isOk();
```
...
```zig
pub fn extract(self: *const DocumentExtractor, input: ExtractInput, config: ExtractionConfig) XbergError!ExtractedDocument
```
...
```zig
pub fn embed(self: *const EmbeddingBackend, texts: []const [:0]const u8) XbergError![]const []const f32
```
...
| `detectedLanguageConfidences` | `\[\]const LanguageConfidence?` | `\[\]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `null` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
```zig
pub fn validate(self: *const ExtractionConfig) XbergError!void
```
...
```zig
pub fn validate(self: *const HeuristicsConfig) XbergError!void
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[:0\]const u8` | — | ISO 639-3 language code, matching the corresponding entry in `detected_languages`. |
...
| `script` | `\[:0\]const u8` | — | Writing system whatlang detected for this language (e.g. `"Latin"`, `"Cyrillic"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `globalLimit` | `f64?` | `null` | Global spend limit in USD. |
| `modelLimits` | `std.StringHashMap(f64)?` | `{}` | Per-model spend limits in USD, keyed by model name. |
| `enforcement` | `\[:0\]const u8?` | `null` | Enforcement mode: `"hard"` (reject over-budget requests) or `"soft"` (log only). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxEntries` | `u64?` | `null` | Maximum number of cached entries. |
| `ttlSeconds` | `u64?` | `null` | Cache entry time-to-live, in seconds. |
| `backend` | `\[:0\]const u8?` | `null` | Cache backend name (e.g. `"memory"`, or an `opendal` scheme). |
| `backendConfig` | `std.StringHashMap(\[:0\]const u8)?` | `{}` | Backend-specific configuration key/value pairs. |
...
| `cooldownSecs` | `u64?` | `null` | Cooldown duration after transient errors, in seconds. Mirrors liter-llm's `LlmConfig.cooldown_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
| `healthCheckSecs` | `u64?` | `null` | Background health check interval, in seconds. Mirrors liter-llm's `LlmConfig.health_check_secs`. Only takes effect when liter-llm's `tower` feature is compiled in; otherwise the value is accepted but unused. |
...
```zig
pub fn validate(self: *const LlmConfig) Error!void
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[:0\]const u8` | — | Provider name, used to key model prefix matching. |
| `baseUrl` | `\[:0\]const u8` | — | Base URL for the provider's OpenAI-compatible API. |
| `authHeader` | `\[:0\]const u8?` | `null` | Header name used to carry the API key (defaults to `Authorization` when unset). |
| `modelPrefixes` | `\[\]const \[:0\]const u8` | `\[\]` | Model name prefixes routed to this provider (e.g. `\["my-provider/"\]`). |
...
| `windowSeconds` | `u64?` | `null` | Rate limit window, in seconds. |
...
```zig
pub fn processImage(self: *const OcrBackend, image_bytes: []const u8, config: OcrConfig) XbergError!ExtractedDocument
```
...
```zig
pub fn processImageFile(self: *const OcrBackend, path: [:0]const u8, config: OcrConfig) XbergError!ExtractedDocument
```
...
```zig
pub fn processDocument(self: *const OcrBackend, path: [:0]const u8, config: OcrConfig) XbergError!ExtractedDocument
```
...
| `backend` | `\[:0\]const u8` | — | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
| `language` | `\[\]const \[:0\]const u8` | `\[\]` | Language code(s) for OCR recognition. Defaults to `\["eng"\]`. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
| `detLimitSideLen` | `u32` | — | Maximum side length for detection image (default: 1024) Larger images may be resized to this limit for faster inference |
...
| `author` | `\[:0\]const u8?` | `/* serde(default) */` | Author/creator of the annotation (PDF `/T` entry). |
| `modified` | `\[:0\]const u8?` | `/* serde(default) */` | Last modification date of the annotation (PDF `/M` entry), as a raw PDF date string (e.g. `"D:20240115120000Z"`). |
| `color` | `\[:0\]const u8?` | `/* serde(default) */` | Annotation colour (PDF `/C` entry), normalised to a CSS-compatible `#rrggbb` hex string. Gray and CMYK colour spaces are converted to RGB. |
| `subject` | `\[:0\]const u8?` | `/* serde(default) */` | Subject of the annotation (PDF `/Subj` entry). |
| `quadPoints` | `\[\]const BoundingBox?` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
| `markedText` | `\[:0\]const u8?` | `/* serde(default) */` | The document text covered by `Self.quad_points`, recovered from the page content underneath the marked-up region. Populated for Highlight, Underline, StrikeOut, and Squiggly annotations when the underlying text could be recovered. |
...
| `layoutGatedPages` | `\[\]const u32?` | `\[\]` | Pages the `auto` layout strategy skipped (1-indexed). `null` unless layout detection ran with `LayoutStrategy.Auto`; empty when the gate selected every page. |
| `layoutGateReasons` | `\[\]const \[:0\]const u8?` | `\[\]` | Why the `auto` layout gate selected or skipped each page. Index `i` is page `i + 1`. Snake_case values such as `multi_column`, `table_grid`, or `plain_text` (the skip reason). `null` unless layout detection ran with `LayoutStrategy.Auto`. |
...
```zig
pub fn process(self: *const PostProcessor, result: ExtractedDocument, config: ExtractionConfig) XbergError!void
```
...
```zig
pub fn renderResult(self: *const Renderer, result: ExtractedDocument) XbergError![:0]const u8
```
...
```zig
pub fn rerank(self: *const RerankerBackend, query: [:0]const u8, documents: []const [:0]const u8) XbergError![]const f32
```
...
| `value` | `\[:0\]const u8?` | `null` | The parsed document as a canonical `serde_json.Value` tree, when the source format could be represented as one. `null` only for TOML inputs whose `toml.Value` fails to round-trip through `serde_json.Value` (xberg-io/xberg#155): the extractor falls back to a raw code block in that case. |
| `flattened` | `\[\]const \[:0\]const u8` | — | Flattened `path: value` renderings for every leaf field, in traversal order. Previously computed and discarded (xberg-io/xberg#166); now surfaced so callers get a full-text view even when the structured renderer only emits headings/lists for a subset of fields. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[\]const \[:0\]const u8` | `\[\]` | Language code(s) for OCR recognition. For Tesseract, languages are joined with "+". A list is the canonical form and the only form accepted by the binding object APIs (Python, Node, PHP, WASM, etc.): `\["eng", "deu"\]`. When deserializing from a config file, JSON body, or the REST/MCP API, a single string is also accepted, either as one code ("eng") or "+"-joined ("eng+deu"). |
...
```zig
pub fn validate(self: *const Validator, result: ExtractedDocument, config: ExtractionConfig) XbergError!void
```
...
| Value | Description |
|-------|-------------|
| `AzureAd` | Azure AD OAuth2 client-credentials flow (Azure OpenAI / Azure Cognitive Services). — Fields: `tenantId`: `\[:0\]const u8`, `clientId`: `\[:0\]const u8`, `clientSecret`: `\[:0\]const u8`, `scope`: `\[:0\]const u8` |
| `VertexOauth2` | Google Vertex AI OAuth2 via a service-account JSON key file on disk. Points at a file path rather than embedding the key inline: the key file contains an RSA private key — stronger secret material than an API key — and `LlmConfig` must never carry that directly, matching the credential-handling policy the rest of this module follows. — Fields: `serviceAccountKeyFile`: `\[:0\]const u8`, `scope`: `\[:0\]const u8` |
| `VertexAdc` | Google Vertex AI Application Default Credentials, resolved from the GCE/GKE/Cloud Run metadata server. Carries no secret material at all. — Fields: `scope`: `\[:0\]const u8` |
| `BedrockWebIdentity` | AWS STS `AssumeRoleWithWebIdentity` (EKS IRSA / OIDC federation) for Bedrock. — Fields: `roleArn`: `\[:0\]const u8`, `tokenFile`: `\[:0\]const u8`, `sessionName`: `\[:0\]const u8`, `region`: `\[:0\]const u8` |
...
| `Comment` | Reviewer/editor comment content (e.g. DOCX comments). Distinct from `NodeContent.Footnote` (xberg-io/xberg#300): comments and footnotes both reach the internal document via a marker/definition pair, but a consumer needs to tell a reviewer comment apart from an authored footnote. — Fields: `text`: `\[:0\]const u8` — **Since:** `v1.1` |
