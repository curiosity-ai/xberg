...
async def main() -> None:
    config = ExtractionConfig(
        structured_extraction=StructuredExtractionConfig(
            ...
            # The Python binding types `schema` as `str` (see _xberg.pyi), so the
            # schema is passed as JSON text rather than a dict.
            schema=json.dumps(
                {
                    "type": "object",
                    "properties": {
    ...
    output = await extract(ExtractInput(uri="paper.pdf"), config)
