...
| `value` | `Option<string>` | Current field value (if filled). |
| `default_value` | `Option<string>` | Default value from the form template. |
...
| `bbox` | `Option<BoundingBox>` | Widget location on the page (x, y, width, height), if known. |
| `max_length` | `Option<u32>` | Maximum input length (text fields only), if specified. |
| `tooltip` | `Option<string>` | Hover text or field description, if present. |
...
- [PDF Extraction](/guides/extraction/) — general PDF text extraction
- [Configuration Reference](/reference/configuration/#pdfconfig) — all PDF options
