//! Integration tests for the `merge_tm_tj_runs` opt-out flag on `SpanMergingConfig`.
//!
...
fn test_merge_tm_tj_disabled_gives_more_spans() {
    ...
    let config_default = SpanMergingConfig::default();
    ...
    for page in 0..page_count {
        ...
        // Text content must be identical regardless of merge flag. ~keep
        ...
        // must be >= the merged count (merging can only reduce the number of spans). ~keep
        assert!(
