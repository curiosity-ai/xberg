...
fn build_constant_cmyk_icc(l_byte: u8) -> Vec<u8> {
    ...
    // a*=0, b*=0 → near-neutral grey through Lab→sRGB). ~keep
...
fn fixture_rgb_then_cmyk_transparent() -> Vec<u8> {
    let icc = build_constant_cmyk_icc(135); // L* ≈ 53 → ~mid-grey ~keep
...
// transform amplifies into byte-distinct RGB. ~keep
...
// that last-paint CMYK as the backdrop. ~keep
...
fn qa_round4_a3_multi_overlap_cmyk_sidecar_carries_last_paint() {
    ...
    // would prove the sidecar bypass. ~keep
    assert_ne!(
        (r, g, b),
        (0, 255, 255),
...
// with known source CMYK, then read the merged plate. ~keep
...
fn qa_round4_a4_opm0_additive_clamp_byte_exact() {
    ...
    // fallback, which for (0.5, 0.5, 1, 0) would emit (128, 128, 0)). ~keep
    assert!(
        r == g && g == b,
...
fn qa_round4_b_byte_identity_ca_fill_alpha_half() {
    ...
    let expected: u64 = 0x993B_0A4A_1B53_B0E5;
    assert_eq!(
        fp, expected,
...
fn qa_round4_b_byte_identity_ca_stroke_alpha_half() {
    ...
    let fp = fingerprint(&rgba);
    let expected: u64 = 0xC7EC_3EFB_9186_A0E5;
    assert_eq!(
        fp, expected,
...
fn qa_round4_b_byte_identity_multiply_red_grey() {
    ...
    let fp = fingerprint(&rgba);
    let expected: u64 = 0xDB92_4170_A70C_39A5;
    assert_eq!(
        fp, expected,
...
fn qa_round4_b_byte_identity_hue_red_over_blue() {
    ...
    let fp = fingerprint(&rgba);
    let expected: u64 = 0x8BAA_5BF7_968C_76C5;
    assert_eq!(
        fp, expected,
...
fn qa_round4_d_smask_4_level_non_cyclic_renders_without_cap_engagement() {
    ...
    // this because render_rgba would have panicked or hung otherwise. ~keep
}
