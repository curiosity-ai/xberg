...
fn build(word_gap_factor: f32, space_w: Option<u32>) -> Vec<u8> {
    ...
    // open a spurious intra-word gap). The non-embedded variant has no /W, so ~keep
    // base 3 is safe.
...
fn extract(pdf: &[u8]) -> String {
    ...
    (0..pages)
        .map(|i| doc.extract_text(i).expect("extract_text"))
        .collect::<Vec<_>>()
        .join(" ")
}
...
fn tight_word_gaps_no_space_glyph_are_spaced() {
    for factor in [0.4_f32, 0.6] {
        ...
        // Lowercase→lowercase word boundaries are the regression-prone case. ~keep
        assert!(
