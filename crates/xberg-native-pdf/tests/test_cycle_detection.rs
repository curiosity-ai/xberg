...
";
    ...
    fn self_referencing_prev_terminates_without_hang() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "xref_self_loop.pdf");
    ...
    fn two_node_prev_cycle_terminates_without_hang() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "xref_two_node_cycle.pdf");
        let _result = PdfDocument::open(&path);
    }
}
...
mod circular_xobject {
    ...
    fn self_referencing_xobject_terminates_without_overflow() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "xobj_self_ref.pdf");
    ...
    fn two_node_xobject_cycle_terminates_without_overflow() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "xobj_two_node_cycle.pdf");
        let doc = PdfDocument::open(&path).expect("Should parse PDF structure");
        let _result = doc.extract_text(0);
    }
    ...
    fn three_node_xobject_cycle_terminates_gracefully() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "xobj_three_node_cycle.pdf");
        let doc = PdfDocument::open(&path).expect("Should parse PDF structure");
        let _result = doc.extract_text(0);
    }
    ...
    fn non_circular_xobject_invoked_twice_produces_text() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "xobj_reused_twice.pdf");
        let doc = PdfDocument::open(&path).expect("Should parse PDF structure");
...
// --- Cyclic structure tree /K references (SIGSEGV regression) --- ~keep
...
mod struct_tree_cycle {
    ...
    fn two_node_struct_cycle_terminates_without_overflow() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "struct_two_node_cycle.pdf");
        let doc = PdfDocument::open(&path).expect("Should parse PDF structure");
        // Must not SIGSEGV or hang — the visited-set guard must break the cycle. ~keep
        let _result = doc.extract_text(0);
    }
    ...
    fn self_referencing_struct_elem_terminates_without_overflow() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "struct_self_ref.pdf");
        let doc = PdfDocument::open(&path).expect("Should parse PDF structure");
        let _result = doc.extract_text(0);
    }
    ...
    fn three_node_struct_cycle_terminates_without_overflow() {
        ...
        let (_path_dir, path) = common::write_temp_pdf(&data, "struct_three_node_cycle.pdf");
        let doc = PdfDocument::open(&path).expect("Should parse PDF structure");
        let _result = doc.extract_text(0);
    }
}
