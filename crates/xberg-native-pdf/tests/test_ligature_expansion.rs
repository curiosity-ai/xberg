...
use xberg_native_pdf::text::ligature_processor::{
...
fn test_ligature_split_after_boundary() {
    ...
    // Expected: ligature should be split because boundary comes after ~keep

    let ligature_fi = create_char_info(0xFB01, 500.0, 0.0, None, true);
    let next_char = create_char_info(0x63, 400.0, 520.0, Some(-150), false);
    // ~keep
...
fn test_ligature_keep_no_boundary() {
    let ligature_fi = create_char_info(0xFB01, 500.0, 0.0, None, true);
    let next_char = create_char_info(0x6E, 400.0, 500.0, None, false);
...
fn test_ligature_ffl_expansion() {
    ...
    // Expected: all three components created with proportional widths ~keep
...
fn test_multiple_ligatures_in_span() {
    let ligature_fi = create_char_info(0xFB01, 500.0, 0.0, None, true);
    let char_l = create_char_info(0x6C, 300.0, 500.0, None, false);
    let char_e = create_char_info(0x65, 400.0, 800.0, Some(-200), false);
...
fn test_ligature_with_geometric_gap() {
    ...
    // Expected: Split because geometric gap indicates boundary ~keep

    let ligature_fi = create_char_info(0xFB01, 500.0, 0.0, None, true);
    // Next char has large gap (500 + 600 = 1100, gap of 600 units) ~keep
    let next_char = create_char_info(0x61, 400.0, 1100.0, None, false);
...
fn test_ligature_decision_boundary_threshold() {
    ...
    // TJ offset threshold is -100 (values more negative indicate boundaries) ~keep
