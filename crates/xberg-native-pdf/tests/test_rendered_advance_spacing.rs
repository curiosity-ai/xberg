...
fn tj_kern_offset_is_separate_synthetic_space_not_in_rendered_advance() {
    ...
    // The kern gap (600/1000 * 12 = 7.2 pt) must NOT be included in 'A's advance. ~keep
    let kern_gap = 600.0_f32 / 1000.0 * 12.0;
    ...
    // 'B' should likewise have rendered_advance ≈ advance_width + Tc ~keep
...
fn horizontal_scaling_multiplies_both_glyph_and_tc_terms() {
    ...
    // If Th were ignored for the Tc term, the delta would be 4.0 instead. ~keep
    let th = 0.5_f32;
    let expected_delta = tc * th;
...
fn flipped_ctm_rendered_advance_is_positive() {
    ...
    // With no Tc/Tw, rendered_advance should equal advance_width ~keep
    assert!(
