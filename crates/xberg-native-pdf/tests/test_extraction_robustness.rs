...
fn encrypted_pdf_page_count_returns_encrypted_error() {
    ...
    // exact condition that triggered the bug. ~keep
    let mut pdf = b"%PDF-1.4\n".to_vec();
    ...
    // What it must NOT do is return Ok(0) — that's the regression. ~keep
    ...
    // the encryption to callers that ask for the structure. ~keep
    assert_eq!(
...
fn rotate_90_keeps_raw_coords_and_does_not_fuse_rows() {
    ...
    // No single token may contain BOTH labels — that is the column-fusion bug. ~keep
    for w in &words {
        assert!(
...
/// Second rotated-frame failure mode: text drawn with a rotated text matrix — `Tm = [0 1 -1 0
...
/// into one giant token / line — the dominant failure on real documents
...
/// Rotated-frame follow-up — a page whose `/Rotate` and content rotation must
