...
fn minimal_shared_xobject_pdf() -> Vec<u8> {
    ...
    //         BT /F0 24 Tf 100 100 Td (Bottom Page) Tj ET ~keep
...
fn shared_xobject_with_per_page_ctm_yields_distinct_page_text() {
    ...
    // (off page — must be filtered by MediaBox clip). ~keep
    ...
    // The two pages' text must not be identical — the whole bug report. ~keep
