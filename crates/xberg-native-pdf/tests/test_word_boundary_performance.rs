...
use xberg_native_pdf::text::word_boundary::{BoundaryContext, CharacterInfo, WordBoundaryDetector};
...
fn create_test_character_array(word_count: usize, chars_per_word: usize) -> Vec<CharacterInfo> {
    ...
    let char_width = 500.0;
    let font_size = 12.0;
    let word_spacing_offset = -150;
...
#[test]
#[ignore] // Performance test with strict timing - run manually, not in CI ~keep
...
fn test_baseline_boundary_detection_large() {
    ...
    // This is acceptable as it enables 2-4× improvement for Latin documents ~keep
    let per_char_micros = stats.avg.as_micros() as f64 / characters.len() as f64;
    assert!(
        per_char_micros < 6.0,
...
fn test_baseline_boundary_detection_scaling() {
    ...
    let sizes = vec![(10, 5), (50, 5), (100, 6), (200, 6), (500, 7)];
...
fn test_baseline_boundary_detection_edge_cases() {
    ...
    for (i, char_info) in large_offsets.iter_mut().enumerate() {
        if i % 5 == 4 {
            char_info.tj_offset = Some(-10000);
        }
    }
...
#[test]
#[ignore] // Timing-sensitive; flaky on shared CI runners ~keep
...
fn try_measure_extraction_mode(
    ...
    for _ in 0..TIMING_ITERATIONS {
        ...
        // Note: mode parameter is for future use when API supports configuration ~keep
        let _ = mode;
...
fn test_baseline_overhead_breakdown() {
    ...
    for _ in 0..100 {
        ...
        let _result: Vec<usize> = boundaries;
    }
