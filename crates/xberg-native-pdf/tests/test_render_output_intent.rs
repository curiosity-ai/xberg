...
const OUTPUT_INTENT_DEFER_PHASE_7_CACHING: &str = "OUTPUT_INTENT_DEFER_PHASE_7_CACHING: plan phase 7 will cache compiled qcms transforms; \
...
/// helper lands.
const OUTPUT_INTENT_DEFER_FORM_XOBJECT_INHERITANCE: &str = "OUTPUT_INTENT_DEFER_FORM_XOBJECT_INHERITANCE: needs a Form XObject test-fixture helper \
...
//   then          output tables (output_channels × 256 bytes) ~keep
...
fn build_minimal_cmyk_to_rgb_lut8_profile_with_version(target_l_byte: u8, version: IccProfileVersion) -> Vec<u8> {
    ...
    // 128 (decoded as the achromatic axis 0). ~keep
...
fn build_minimal_gray_trc_profile() -> Vec<u8> {
    ...
    // gamma table for the gray channel. ~keep
    let entry_count: u32 = 256;
    let mut curv = Vec::with_capacity(12 + (entry_count as usize) * 2);
    curv.extend_from_slice(&0x6375_7276u32.to_be_bytes());
    curv.extend_from_slice(&0u32.to_be_bytes());
    ...
    // curveType body. Tag data offset = 144. ~keep
    let mut profile = vec![0u8; 128];
    ...
    profile.extend_from_slice(&1u32.to_be_bytes());
    profile.extend_from_slice(&0x6b54_5243u32.to_be_bytes());
    profile.extend_from_slice(&144u32.to_be_bytes());
    profile.extend_from_slice(&(curv.len() as u32).to_be_bytes());
...
fn build_pdf_default_cmyk_overrides_output_intent(
    ...
    // §8.6.5.6. Its value is an ICCBased colour space wrapping profile B. ~keep
    ...
    // redirects through /DefaultCMYK. ~keep
...
fn build_pdf_default_gray_routes_bare_device_gray() -> Vec<u8> {
    ...
    // magenta-channel-only pixel. ~keep
...
/// converter (`xberg_native_pdf::color::cmyk_to_rgb`, tetralinear over the 16
...
fn device_cmyk_paint_with_output_intent_renders_via_icc_not_additive_clamp() {
    ...
    // composite renderer will exercise on this profile. ~keep
    {
        use std::sync::Arc;
...
/// parameter for CMYK inputs altogether.
#[test]
...
///
/// This surfaces an open design question as a probe: when
...
fn output_intent_with_unparseable_profile_falls_through_to_process_inks() {
    ...
    // unit test in color.rs uses but reaches the rasteriser end-to-end. ~keep
...
fn output_intent_with_mismatched_icc_header_colour_space_is_rejected_at_parse() {
    ...
    header_only[8..12].copy_from_slice(&0x0400_0000u32.to_be_bytes());
    header_only[12..16].copy_from_slice(b"prtr");
    header_only[16..20].copy_from_slice(b"RGB ");
    header_only[20..24].copy_from_slice(b"Lab ");
    header_only[36..40].copy_from_slice(b"acsp");
    ...
    // Renderer falls through to §10.3.5 byte-for-byte. ~keep
    let rgba = render_rgba(&doc);
    let (r, g, b, a) = pixel_at(&rgba, 50, 50);
    assert_eq!(
        (r, g, b, a),
...
fn process_ink_consistency_between_extractors_helper_and_no_output_intent_arm() {
    ...
    fn spec_process_ink(c: u8, m: u8, y: u8, k: u8) -> [u8; 3] {
        let (r, g, b) = xberg_native_pdf::color::cmyk_to_rgb(
            c as f32 / 255.0,
            m as f32 / 255.0,
            y as f32 / 255.0,
            k as f32 / 255.0,
...
fn page_level_default_cmyk_takes_precedence_over_output_intent() {
    ...
    // fire for the wrong reason. ~keep
    {
        use std::sync::Arc;
        use xberg_native_pdf::color::{IccProfile, RenderingIntent, Transform};
...
/// (`PageRenderer::icc_transform_cache_build_count`), a `#[doc(hidden)]`
/// accessor that exposes renderer internals for assertions. Reading the
/// per-instance counter avoids racing other concurrent integration
/// tests that might also call `Transform::new_srgb_target` on the same
/// process — the cache is local to the `PageRenderer` we construct
/// here, so nobody else touches it.
///
...
/// xberg-native-pdf parses this and the colour stage threads it through
...
// at the xberg-native-pdf layer, and the cache key separates entries by intent
...
//      /RelativeColorimetric) is honoured at the colour boundary. ~keep
...
fn qa_round3_unknown_intent_name_falls_back_to_relative_colorimetric() {
    use xberg_native_pdf::color::RenderingIntent;

    // §8.6.5.8: unrecognised → RelativeColorimetric. ~keep
    assert_eq!(
...
// profile uses a constant CLUT. ~keep
...
fn embedded_iccbased_n4_trumps_document_output_intent() {
    ...
    // assertion below fire for the wrong reason. ~keep
    {
        use std::sync::Arc;
        use xberg_native_pdf::color::{IccProfile, RenderingIntent, Transform};
        let prof_a = Arc::new(IccProfile::parse(profile_a.clone(), 4).expect("parse A"));
...
// confirms the probes do what their names say. ~keep
...
//      is invoked for separations on the same fixture. ~keep
...
fn qa_round2_iccbased_n4_with_embedded_profile_routes_cmyk_to_plates() {
    ...
    let sample = |p: &xberg_native_pdf::rendering::SeparationPlate| {
...
fn qa_round2_iccbased_n3_with_cmyk_output_intent_ignores_output_intent() {
    ...
    // bytes; parse will fail (no acsp) and the fallback path fires. ~keep
    let icc_b_off = buf.len();
...
fn qa_round3_iccbased_v4_output_intent_drives_render_through_qcms() {
    ...
    // fires → 191,255,255 instead of ~128). ~keep
    {
        use std::sync::Arc;
        use xberg_native_pdf::color::{IccProfile, RenderingIntent, Transform};
...
//
// What this means for xberg-native-pdf:
...
// grows BPC (either via fork or upgrade) OR xberg-native-pdf switches to a
...
const HONEST_GAP_QCMS_030_NO_BPC: &str = "HONEST_GAP_QCMS_030_NO_BPC: qcms 0.3.0 ignores rendering intent for CMYK \
...
fn qa_round4_bpc_paper_white_preservation_under_relative_colorimetric() {
    ...
    // four-intent assertion below holds for any LUT. ~keep
...
fn build_minimal_cmyk_to_rgb_lut8_profile_with_shadow_ramp(l_range: std::ops::RangeInclusive<u8>) -> Vec<u8> {
    ...
    // simplicity (the LUT8 path then drives shadow ramp linearly on K). ~keep
...
// press-target value." That's what a real press profile would prove. ~keep
...
///      clamp value for the green CMYK build. That's the pre-fix
///      baseline.
...
fn qa_round4_branding_green_mark_routes_through_output_intent() {
    ...
    // emitting the raw additive-clamp value. ~keep
...
/// the proxy.
const HONEST_GAP_NO_REAL_BRANDING_FIXTURE: &str = "HONEST_GAP_NO_REAL_BRANDING_FIXTURE: no real branding-logo PDF on disk; \
...
// OutputIntent as CMYK) would flip them RED and surface the regression. ~keep
...
fn output_intent_n3_rgb_profile_rejected_at_reader_falls_through_to_process_inks() {
    ...
    // ever invokes IccProfile::parse on the stream payload. ~keep
    let mut buf: Vec<u8> = Vec::new();
    buf.extend_from_slice(b"%PDF-1.4\n");
    let cat_off = buf.len();
...
fn output_intent_with_outputcondition_string_only_no_destoutputprofile_falls_through() {
    ...
    // does not carry the ICC bytes itself. ~keep
    let catalog_entries = "/OutputIntents [<< /Type /OutputIntent /S /GTS_PDFX /OutputCondition (FOGRA39 (ISO 12647-2:2004)) /OutputConditionIdentifier (FOGRA39) >>]";
    let content_ops = "0.25 0 0 0 k\n20 20 60 60 re\nf\n";
...
fn cmyk_transparency_compose_hoists_icc_transform_cache_lookup() {
    ...
    // a small constant per paint (one for the with_coverage helper). ~keep
...
// `tracing::warn!` on that surface so the silent degradation is
// observable until the upstream fix for the swallowed
// profile-parse diagnostic lands.
// =========================================================================== ~keep

/// Capture warn-level tracing events into an in-test buffer so the probe
/// can grep for the H3b diagnostic. Mirrors `CapturingLayer` in
/// `src/rendering/sidecar.rs::tests` — duplicated here because the
/// integration test crate can't reach in-crate test types.
///
/// Installed per-call via `tracing::subscriber::with_default`, which is
/// thread-scoped, so no global install and no buffer-offset bookkeeping.
...
fn h3b_rgb_paint_under_unparseable_output_intent_logs_k_zero_warning() {
    ...
    assert!(
        ...
        "H3b: an RGB paint under a malformed /OutputIntents declaration \
         must emit a `tracing::warn!` naming the K=0 fallback and the \
         upstream issue it hangs on. Captured records since start: {:?}",
        records
    );
}
