//! Glyph-order transposition fragments on multi-column textbooks.
//!
//! An earlier rowspan fix dramatically reduced the count of garbled
//! fragments (`aaxons`, `acclipmoshed`, `accpamonies`, `achrmaotic`) on
//! dense textbooks, but residual transpositions remain. The original
//! hypothesis was that a sort comparator in the reading-order pipeline
//! is unstable or that its X-coordinate tolerance is too loose.
//!
//! Fixture diagnosis note: we validated extract_text on a
//! dense multi-column genetics textbook from `pdfs_slow3/` and saw
//! fragments like `accompaally`, `aaand`. Tracing
...
//! textbooks is still open and needs deeper column-detection work.
//!
//! Quantified impact: the Hartwell Genetics textbook
...
fn out_of_order_glyph_emission_sorts_visually() {
    ...
    // reading-order sort puts `a` before `x` because x=100 < x=107. ~keep
    let content = "\
