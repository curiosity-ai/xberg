...
fn marker_quadrant(rotate: i32) -> (bool, bool) {
    ...
    // RAW pixels, not the default PNG-encoded bytes. ~keep
...
fn rotate_places_the_corner_marker_by_turning_not_mirroring() {
    ...
    // (ISO 32000-1 s7.7.3.3), so the corner walks: TL -> TR -> BR -> BL. ~keep
    ...
    // The regression: 270 used to land TOP-left (a mirror of the truth). ~keep
