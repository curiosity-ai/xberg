...
fn truncated_pdf(keep_pages_root: bool) -> Vec<u8> {
    ...
    // Everything from here on is what a tail truncation destroys. ~keep
    let truncate_at = buf.len();

    // Page-tree root (obj 2) and Catalog (obj 1), last. ~keep
    ...
    if keep_pages_root {
        ...
        // Cut off the Pages root too - fully orphaned pages. ~keep
...
fn truncated_pdf_pages_in_objstm() -> Vec<u8> {
    ...
    // never live inside an ObjStm, so these are always at real offsets). ~keep
    let obj = |buf: &mut Vec<u8>, off: &mut Vec<usize>, id: usize, body: &str| {
        off[id] = buf.len();
        buf.extend_from_slice(format!("{id} 0 obj\n{body}\nendobj\n").as_bytes());
    };
    ...
    // first dict begins, /N is the count. No /Filter -> the bytes are literal. ~keep
    ...
    // Catalog, the xref stream and the trailer. ~keep
    let truncate_at = buf.len();
    obj(&mut buf, &mut off, 2, "<< /Type /Pages /Kids [3 0 R 4 0 R] /Count 2 >>");
    obj(&mut buf, &mut off, 1, "<< /Type /Catalog /Pages 2 0 R >>");
    let xref = buf.len();
