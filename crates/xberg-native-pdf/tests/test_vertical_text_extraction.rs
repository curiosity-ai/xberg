...
CMapName currentdict /CMap defineresource pop
    ...
    // downward) at the spec-default vertical origin (500, 880). ~keep
    let o6 = pdf.len();
    pdf.extend_from_slice(
...
fn vertical_identity_v_emits_y_advancing_spans() {
    ...
    // glyph's user-space origin is preserved. ~keep
    ...
    // vertical case, since horizontal advances accumulate along X. ~keep
    ...
    let x_extent = |spans: &[xberg_native_pdf::layout::TextSpan]| -> f32 {
    ...
    let y_extent = |spans: &[xberg_native_pdf::layout::TextSpan]| -> f32 {
