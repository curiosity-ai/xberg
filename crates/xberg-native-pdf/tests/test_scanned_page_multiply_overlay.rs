...
//!
//! Fixture `538250-1.pdf` was pulled from the pdf.js issue tracker
//! (which reproduced the same regression in pdf.js). We render it at 96 DPI
...
const FIXTURE: &str = "tests/fixtures/regressions/alpha_channel/538250-1.pdf";
...
fn renders_without_error() {
    ...
    // regression would leave a small, near-empty buffer. ~keep
    assert!(
        img.data.len() > 1024,
