...
//!
//! The defect: a scanned Hebrew OCR PDF (invisible `Tr 3` text layer,
...
//! character) that this fix does **not** address — it lives in a
//! different part of the pipeline (`merge_adjacent_spans`) and needs its
//! own, separately render-mode-gated fix. Confirmed via the full 419-PDF
//! corpus regression sweep that naively fixing the per-glyph shape
//! regresses ordinary, non-OCR Hebrew/Arabic PDFs (Wikipedia-style
//! exports) by reversing already-correct text — so this fix's scope is
//! intentionally narrower: real bug, zero regressions.
//!
...
//! the shape the reporter described and the one that exposed
...
// 20 words, 84 unique codepoints, page 612.0x792.0pt (8.5x11in @ 300dpi). ~keep
...
/// exposed the bug (`process_tj_array_tiebreaker`'s buffer never persists
...
/// extraction — the core regression guard for this defect, broadened to every
