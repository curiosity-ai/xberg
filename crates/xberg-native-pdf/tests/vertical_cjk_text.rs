...
endcmap CMapName currentdict /CMap defineresource pop end end";
    ...
    // (24pt pitch with 18pt glyphs → square-ish vertical stacking). ~keep
...
/// Vertical CJK must read column-major as prose. (The raw spatial
/// `extract_tables` detector does see a grid here — the prose rejection that
/// suppressed it lived in the removed converter path, so this pins the
/// reading order only.)
#[test]
fn vertical_cjk_is_column_major() {
    let expected = "\u{4E00}\u{4E8C}\u{4E09}\u{56DB}\u{4E94}\u{516D}\u{4E03}\u{516B}\u{4E5D}";
    let doc = PdfDocument::from_bytes(vertical_cjk_pdf()).unwrap();

    let text = doc.extract_text(0).unwrap();
    let compact: String = text.chars().filter(|c| !c.is_whitespace()).collect();
    assert_eq!(compact, expected, "vertical CJK not column-major: {text:?}");
}
