//! SMask, knockout and transparency-group flattening scenarios.
//!
//! Coverage:
//!   - SMask cache CTM invalidation + SMask under nested Do
//!   - SMask clipping across image and text paints
//!   - spec compliance + malformed-input hardening
//!   - SMask + knockout review-feedback hardening
//!
//! Probes use byte-exact references where the spec admits one
//! (knockout byte-equality) and bounded-band assertions where the
//! discriminator is colour-channel separation (overprint plate
//! retention). Idiom: synthetic-PDF builder, raw-RGBA render, pixel
//! sampling — no rendered-image diff baselines.
...
// ===========================================================================
// C.1 — SMask cache CTM invalidation + SMask under nested Do
// ===========================================================================
//
// Two scenarios.
//
...
// rasterise against the page-sized pixmap so subsequent paints align. ~keep

/// Cache CTM invalidation.
///
...
fn smask_cache_invalidates_when_ctm_changes() {
    ...
    // expose any cache-staleness on the SMask CTM. ~keep
    ...
    // (PDF 10..40, image y = 100-40..100-10 = 60..90). ~keep
    ...
    // (65, 25) (PDF 50..80 ⇒ image y = 100-80..100-50 = 20..50). ~keep
...
/// SMask installed *inside* a Form XObject invoked via Do.
///
...
fn smask_applies_to_paint_inside_nested_do() {
    ...
    assert!(
        ...
        "SMask inside nested Do: expected ~(255, 128, 128); got \
         ...
         clip F1's own paints."
    );
}

// ===========================================================================
// C.2 — SMask clipping across image and text paints
// ===========================================================================
//
// Checks that an active SMask clips non-path paint operators
// correctly. ~keep

/// Active SMask clips text paint.
///
...
// ===========================================================================
// C.3 — Malformed SMask inputs must not panic
// ===========================================================================
//
...
//
// On THIS branch the hardening fixes did NOT land, so some probes
// may surface bugs (panic / hang / undefined behaviour). Those are
// real bugs to flag. ~keep

/// Malformed: /SMask missing /S subtype.
///
...
#[test]
fn smask_missing_s_subtype_does_not_panic() {
    let smask_form = "0.5 g\n0 0 100 100 re\nf\n";
    let obj_5 = format!(
        "5 0 obj\n<< /Type /XObject /Subtype /Form /BBox [0 0 100 100] \
         /Resources << >> /Length {} >>\nstream\n{}\nendstream\nendobj\n",
...
#[test]
fn smask_bc_out_of_range_does_not_panic() {
    let smask_form = "0.5 g\n0 0 100 100 re\nf\n";
    let obj_5 = format!(
        "5 0 obj\n<< /Type /XObject /Subtype /Form /BBox [0 0 100 100] \
         /Resources << >> /Length {} >>\nstream\n{}\nendstream\nendobj\n",
    ...
    // /BC carries values outside [0, 1] (incl. negative and >1). ~keep
    let content = "1 1 1 rg\n0 0 100 100 re\nf\n\
                   /Sm gs\n\
                   1 0 0 rg\n\
                   10 10 80 80 re\nf\n";
...
// ===========================================================================
// C.4 — SMask + knockout review-feedback hardening
// ===========================================================================
//
// The audit suite already covers basic knockout
// (HONEST_GAP_GROUP_KNOCKOUT is un-ignored). The scenarios unique to
// this file:
//
...
//
// Port the ones not already covered. ~keep

/// `/Group` as an indirect reference.
///
...
#[test]
fn group_indirect_ref_resolves_transparency_flag() {
    ...
    // avoid false flags on the half-implemented isolation path. ~keep
    ...
    // isolation, the form paints blue). ~keep
...
/// Knockout under non-Normal blend mode.
///
...
/// is wrong). The fix added `knockout_paint_alpha(gs_alpha,
...
#[test]
fn knockout_under_multiply_blend_redirects_to_backdrop() {
    ...
    assert_eq!(
        ...
        "Knockout group under /BM /Multiply must redirect element to \
         ...
         skipping knockout redirect."
    );
}

/// Knockout pixel-exact byte-equality.
///
...
#[test]
fn knockout_byte_equal_under_full_coverage() {
    ...
    // Assertion: A and B are byte-identical in the painted region. ~keep
    let form_a = "1 0 0 rg\n0 0 100 100 re\nf\n\
                  0 0 1 rg\n0 0 100 100 re\nf\n";
    let form_b = "0 0 1 rg\n0 0 100 100 re\nf\n";
    let obj_5_a = format!(
