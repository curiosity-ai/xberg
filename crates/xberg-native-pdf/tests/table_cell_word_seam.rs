...
fn fragmented_cell_pdf() -> Vec<u8> {
    ...
    // past the declared end of the previous one. ~keep
    ...
    // Continuation at the exact pen position: 119.808 + 2 * 4.077 = 127.962. ~keep
...
fn seam_is_wider_than_the_word_space_it_must_be_told_apart_from() {
    ...
    // "Cre" -> "d": pure repositioning between consecutive glyphs. ~keep
    let seam = gap_after("Cre");
    // "it" -> "<": a real space glyph sits between them in the source. ~keep
    ...
    // (it re-glues split words; it does not re-split fused ones). ~keep
    assert!(
        space > 9.0 * 0.15,
...
fn sub_em_fragment_seams_do_not_split_cell_words() {
    ...
    // that the merger itself inserts a space), fail here, loudly. ~keep
...
fn sub_em_fragment_seams_do_not_split_words_in_extracted_text() {
    ...
    // text and would pass with the fix reverted. ~keep
    assert!(
...
fn build_minimal_pdf_raw(content: &[u8], page_extra: &[u8]) -> Vec<u8> {
    ...
    // font_size * 0.15 floor so a correct implementation keeps it. ~keep
    let off5 = pdf.len();
