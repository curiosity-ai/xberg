...
fn b1_imagemask_do_inside_k_knockout_last_paint_wins_byte_exact() {
    ...
    // default /Decode [0 1] where bit 0 = opaque-paint-with-fill). ~keep
    ...
    // group's initial backdrop 0 at full alpha. ~keep
    assert_eq!(
        pms185, 179,
...
//   In f32 0.5·0.6000000238 = 0.30000001 → ×255 = 76.500003 → u8 77. ~keep
// ===========================================================================
...
fn b2_pattern_with_separation_underlying_writes_spot_lane_byte_exact() {
    ...
    // Pattern recursion under test; using m/l/h/f bypasses the ~keep
...
// the spot's tint transform contribution.) ~keep
// ===========================================================================

#[test]
fn b3_composite_preview_separation_tint_transform_byte_exact() {
    let icc = build_constant_cmyk_icc(135);
    // Tint transform: linear 0.0 → (1,1,1) → no ink, 1.0 → (0,1,0) ~keep
    ...
    //
    // /ca 0.5 attenuates the paint contribution at the §11.3.3 ~keep
    // compose step.
    let content = "/CS_S cs\n/Ov gs\n0.5 scn\n0 0 100 100 re\nf\n";
    let resources = format!(
        "/ExtGState << /Ov << /Type /ExtGState /ca 0.5 >> >> \
    ...
    // → u8 round = 191. Byte-exact references are 191/255/191.) ~keep
    assert_eq!(
        r, 191,
...
//   - ISO 32000-1 §8.7.3.1 — Pattern colour space underlying ~keep
// ===========================================================================
...
fn b2_pattern_with_separation_underlying_indirect_ref_byte_exact() {
    ...
    // object before semantic interpretation). ~keep
    assert_eq!(
        pms185, 77,
