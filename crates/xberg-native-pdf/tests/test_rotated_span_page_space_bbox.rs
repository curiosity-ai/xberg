...
fn rotated_run_reports_a_page_space_box() {
    ...
    for s in rotated {
        ...
        // redefinition, so nothing reading `bbox` today moves. ~keep
        assert!(
        ...
        // land the box at [200, 210], on the wrong side of the baseline. ~keep
        assert!(
...
/// The same sideways run on a `/Rotate 90` page — the sideways-landscape
...
fn rotate90_page_span_page_box_is_not_double_transformed() {
    ...
    for s in rotated {
        ...
        // the mapped rect's top edge (the image of the run origin). ~keep
        assert!(
