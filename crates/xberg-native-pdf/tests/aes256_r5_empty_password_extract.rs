...
//! primitives directly — deliberately not through xberg-native-pdf's own key
...
const FILE_KEY: [u8; 32] = [
    0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7, 0xC0, 0xC1, 0xC2,
    0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7,
];
...
fn r5_encrypted_pdf() -> Vec<u8> {
    // U: SHA256(pw ‖ validation salt) ‖ validation salt ‖ key salt. ~keep
    ...
    // UE: AES-256-CBC(key = SHA256(pw ‖ key salt), iv = 0) over the file key. ~keep
    let ue_entry = aes256_cbc_nopad(&sha256(&[b"", &USER_KEY_SALT]), &[0u8; 16], &FILE_KEY);

    // O: SHA256(pw ‖ owner validation salt ‖ U) ‖ owner validation salt ‖ owner key salt. ~keep
    ...
    // OE: AES-256-CBC(key = SHA256(pw ‖ owner key salt ‖ U), iv = 0) over the file key. ~keep
    let oe_entry = aes256_cbc_nopad(&sha256(&[b"", &OWNER_KEY_SALT, &u_entry]), &[0u8; 16], &FILE_KEY);

    // Perms: P (little-endian) ‖ FF FF FF FF ‖ 'T' ‖ "adb" ‖ 4 arbitrary bytes,
    // AES-256 encrypted with the file key. A single block with iv = 0 is ECB. ~keep
    ...
    // Content stream, AES-256-CBC with the IV prepended (§7.6.2). ~keep
    ...
    let mut bodies: Vec<Vec<u8>> = vec![Vec::new(); 7];
