...
// modules so we re-declare the helpers we need. ~keep
...
// immediately after the inner B replacement. ~keep
...
fn fixture_major1_nested_bdc() -> Vec<u8> {
    ...
    // producing "A B D C" instead of "A B C D". ~keep
    let content = b"BT\n/F1 12 Tf\n50 700 Td\n\
        /Span << /MCID 10 >> BDC\n\
          (A) Tj\n\
          /Span << /MCID 11 >> BDC\n\
    ...
    b.register_mcid(0, 10, 9);
    b.register_mcid(0, 11, 10);
    b.register_mcid(0, 12, 11);
    b.build()
}
...
fn major1_nested_bdc_restores_outer_mcid() {
    ...
    // and stays inside P-A's group, which precedes P-D in the tree. ~keep
