//! Round-1 QA probes for the CMYK + spot-ink sidecar.
//!
...
use xberg_native_pdf::rendering::sidecar::{BlendModeClass, SpotBlendDispatch};
...
pub const QA_BUG_DEVICEN_PROCESS_POLLUTES_SPOT_SET: &str = "QA_BUG_DEVICEN_PROCESS_POLLUTES_SPOT_SET: ISO 32000-1 §8.6.6.5 \
...
/// instead of `Multiply`.
pub const QA_BUG_BM_ARRAY_NOT_HONOURED: &str = "QA_BUG_BM_ARRAY_NOT_HONOURED: ISO 32000-1 §11.3.5 + §11.6.3: a \
...
pub const QA_GAP_DISCOVER_ERROR_SURFACED_VIA_WARN: &str = "QA_GAP_DISCOVER_ERROR_SURFACED_VIA_WARN: round-1 fix landed: \
...
fn qa1_1_devicen_with_process_subtype_excludes_process_channels() {
    ...
    // is a true spot. (§8.6.6.5 Table 73: `Subtype` / `Process` keys.) ~keep
...
fn qa2_2_detection_does_not_fire_for_bm_array_unwrapping_to_normal() {
    ...
    // fix to the array handling does not break it. ~keep
    assert!(
...
fn qa3_1_case_sensitive_mode_names_fall_back_to_normal() {
    ...
    // an unknown name and falls back to Normal's class. ~keep
...
fn qa3_5_compatible_mode_legacy_normal_synonym() {
    ...
    // /Compatible as a known synonym. ~keep
    assert_eq!(
...
fn qa3_6_hex_escaped_name_post_parser_resolution() {
    ...
    // a recognised blend mode name and falls back to Normal class. ~keep
    assert_eq!(
...
fn qa5_1_separation_named_after_process_colorant_is_still_a_spot() {
    ...
    // declaration mechanism per §8.6.6.4). ~keep
    let resources = "/ExtGState << /Half << /Type /ExtGState /ca 0.5 >> >> \
...
/// tolerance bands; round 1 of the sidecar work must hold the same
/// line. This is a compile-time hint via documentation — the actual
/// scan is done by grep in the QA report. The probe documents the
/// intent.
#[test]
fn qa7_1_tolerance_band_guard_documents_intent() {
    ...
    // grep still returns no results. ~keep
}
