...
fn build_nonlinear_cmyk_to_lab_lut8_profile() -> Vec<u8> {
    ...
    // 255) gives a gamma-2.2 forward curve that lifts mid-tones. ~keep
    for _ in 0..in_chan {
        for i in 0..256u16 {
    ...
    // curve above makes the overall mapping non-linear. ~keep
    ...
    for idx in 0..grid_size {
        ...
        // w = last channel = K innermost). So idx = c*8 + m*4 + y*2 + k. ~keep
        let c = (idx >> 3) & 1;
        let m = (idx >> 2) & 1;
        let y = (idx >> 1) & 1;
        let k = idx & 1;
...
fn build_pdf_with_optional_output_intent(
    ...
    for obj in extra_objs {
        ...
        // assume the caller numbered them starting at `next_obj_num`. ~keep
        let _ = next_obj_num;
...
// CMYK(0.5,0.5,0.5,0.5) must produce visibly different RGB. ~keep
...
//
// Convert-first ordering (current xberg-native-pdf behaviour):
//
//   for each paint:
...
// compose-first value — the probe fails, surfacing the gap. ~keep
...
fn fixture_nonlinear_icc_two_overlapping_cmyk_paints() -> Vec<u8> {
    ...
    // quadruple. If convert-first (current code), it equals the ~keep
...
fn qa_round2_compose_before_convert_under_nonlinear_icc() {
    ...
    // image y=100-40 = 60). Sample a 20×20 mean to swamp AA noise. ~keep
    ...
    // discrimination check — should the implementation regress, we want ~keep
    ...
    // entirely inside the painted rect. ~keep
    assert_eq!(
    ...
    // deterministic — so byte-exact equality holds. ~keep
    assert_eq!(
...
// or overprint effect, which it will not as-shipped. ~keep
...
fn qa_round2_smask_modulates_fill_stroke_combo() {
    ...
    // path surfaces as a value change. ~keep
    assert_eq!(
        (r, g, b),
        (255, 127, 127),
...
fn fixture_overprint_for_op(op_ops: &str) -> Vec<u8> {
    ...
    // the yellow knocks the cyan out completely. ~keep
    let content = format!(
        "0.5 0 0 0 k\n10 10 80 80 re\nf\n\
         /OpOn gs\n\
         0 0 1 0 k\n\
...
fn fixture_smask_scoped_through_q_then_paint_outside() -> Vec<u8> {
    ...
    // again outside scope (red WITHOUT SMask → fully opaque red). ~keep
...
fn qa_round2_smask_does_not_leak_across_q_q() {
    ...
    // the paint-arm coverage probes hit. ~keep
    assert_eq!(
        (r_in, g_in, b_in),
        (255, 127, 127),
...
// lossy. The probe pins the magnitude of the loss. ~keep
...
// SMask + overprint behaviour. ~keep
...
fn qa_round3_smask_modulates_do_form_xobject() {
    ...
    // Without wiring, Do paints fully opaque red ⇒ (255, 0, 0). ~keep
    assert_eq!(
        (r, g, b),
        (255, 127, 127),
...
fn qa_round3_smask_modulates_paint_shading() {
    ...
    // Painted region (PDF 20..80, 20..80) ⇒ image (20..80, 20..80). ~keep
    let (r, g, b, _) = pixel_at(&rgba, 50, 50);
    assert_eq!(
        (r, g, b),
        (255, 127, 127),
...
fn qa_round2_overprint_reconstruction_under_nonlinear_icc() {
    ...
    // (centre of overlap) is uniformly inside the painted rect. ~keep
    ...
    // loss. The Priority-4 plate-retention fix drives delta to zero. ~keep
    assert_eq!(
        actual, press,
