...
fn fixture_body_two_column_no_interleave() {
    ...
    // No interleave: between two consecutive left lines, no right line. ~keep
...
// TODO: Re-enable once the table-detector density gate tightens.
// Accurate standard-14 font widths expose a pre-existing
// false-positive in the spatial table detector on mixed-size
// two-column body text — the detector fires because widths are
// now accurate and the word-grid looks table-shaped. ~keep
#[ignore]
#[test]
...
// =====================================================================
// Multi-column interleave invariants.
//
...
/// interleave case (`correlaanonymous`). `correla-` / `tion` hyphenation
...
/// in left-column text — the failure pattern the column-detection fix
/// addressed on real CFR PDFs.
#[test]
...
fn fixture_fax_scattered_fragments_no_reversal() {
    ...
    // from mis-routing this page as multi-column. ~keep
    let lines: &[&[(f32, &str)]] = &[
        &[
            (72.0, "WORDONE"),
            (140.0, "WORDTWO"),
    ...
    // of row 2 — proves we're reading row-major, not column-major. ~keep
