//! Spec-correct structure-tree surfacing in `extract_structured`
...
fn tagged_two_chapter_pdf() -> Vec<u8> {
    ...
    // extractor can resolve page content back to the tree. ~keep
    obj(
        &mut buf,
        &mut off,
        8,
...
/// An `Lbl` structure element becomes a `MarginalLabel` region.
#[test]
fn lbl_structure_elements_become_marginal_labels() {
    use xberg_native_pdf::RegionRole;
...
/// Every region carries its `Sect` section index; chapter 1 keeps
...
fn sections_are_stable_across_pages_and_separate_chapters() {
    ...
    let sect_of = |page: &xberg_native_pdf::StructuredPage, needle: &str| -> Option<usize> {
        page.regions
            .iter()
