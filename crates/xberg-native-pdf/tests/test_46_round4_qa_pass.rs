...
pub const HONEST_GAP_DEVICEN_PROCESS_OVERPRINT_CLASS: &str = "HONEST_GAP_DEVICEN_PROCESS_OVERPRINT_CLASS: a DeviceN / NChannel \
...
fn devicen_process_subtype_routes_to_process_class() {
    ...
    // source tints feed all four process channels via /Process /CMYK). ~keep
    assert!(
        m > 0 && y > 0 && k > 0,
...
fn nchannel_process_subtype_routes_to_process_class() {
    ...
    // requirements; the /Process /CMYK reconstruction path is the same. ~keep
    let c = centre(plate(&plates, "Cyan"));
    let m = centre(plate(&plates, "Magenta"));
    let y = centre(plate(&plates, "Yellow"));
    let k = centre(plate(&plates, "Black"));
...
fn render_pure_op_devicecmyk_walker() -> Vec<xberg_native_pdf::rendering::SeparationPlate> {
    let icc = build_constant_cmyk_icc(135);
    let content = "0.4 0 0 0 k\n0 0 100 100 re\nf\n\
                   /Ov gs\n0 0.5 0 0 k\n0 0 100 100 re\nf\n";
...
/// path.
fn render_pure_op_devicecmyk_composite() -> Vec<xberg_native_pdf::rendering::SeparationPlate> {
    let icc = build_constant_cmyk_icc(135);
    let content = "0.4 0 0 0 k\n0 0 100 100 re\nf\n\
                   /Ov gs\n0 0.5 0 0 k\n0 0 100 100 re\nf\n";
...
fn devicergb_opm1_no_zero_preserve_byte_exact() {
    ...
    // 0.5·0 + 0.5·0.30196 = 0.15098 → round(0.15098·255) = round(38.5) = 39. ~keep
