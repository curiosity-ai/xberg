...
fn build_synthetic_japan1_pdf() -> Vec<u8> {
    ...
    // UniJIS-UCS2-H table. Both are present in Droid Sans Fallback. ~keep
...
fn font_info_flags_predefined_cidfont_for_substitution() {
    ...
    // bundled-font availability. ~keep
    ...
    // proves the FontInfo construction path didn't trip on the new field. ~keep
...
fn synthetic_adobe_japan1_pdf_renders_non_blank() {
    ...
    // below this) if substitution silently regresses to a blank page. ~keep
    assert!(
        non_white >= 1000,
...
fn jo_pdf_vertical_japanese_renders_non_blank() {
    ...
    // content region (the document has no other glyphs). ~keep
    assert!(
        non_white >= 2000,
...
fn kampo_pdf_japanese_pharmacopeia_renders_non_blank() {
    ...
    // more ink — a paragraph or two of Japanese text per page. ~keep
    assert!(
        non_white >= 5000,
