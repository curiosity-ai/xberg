...
mod gap_extraction_tests {
    use super::*;
    use xberg_native_pdf::extractors::analyze_document_gaps;
    ...
    fn test_extract_gaps_single_line() {
        ...
        // The gaps extracted would be: [2.0] (gap between span 0 and span 1) ~keep
    ...
    fn test_extract_gaps_including_overlaps() {
        let spans = vec![
            ...
            create_test_span("word2", 10.0, 0.0, 10.0, 12.0),
            create_test_span("word3", 22.0, 0.0, 10.0, 12.0),
        ];
        assert_eq!(spans.len(), 3);

        let gap1 = spans[1].bbox.left() - spans[0].bbox.right();
        assert_eq!(gap1, -2.0);

        let gap2 = spans[2].bbox.left() - spans[1].bbox.right();
        assert_eq!(gap2, 2.0);
    }
...
mod threshold_determination_tests {
    ...
    fn test_determine_threshold_clamping_min() {
        ...
        if result.stats.is_some() {
            // 0.02 * 1.5 = 0.03, should clamp to 0.05 ~keep
    ...
    fn test_determine_threshold_clamping_max() {
        ...
        if result.stats.is_some() {
            // 1.5 * 1.5 = 2.25, should clamp to 1.0 ~keep
...
mod mixed_document_tests {
    ...
    fn test_mixed_spacing_document() {
        let mut gaps = vec![0.15, 0.2, 0.18, 0.22, 0.17, 0.19, 0.16, 0.21];
        gaps.extend_from_slice(&[8.0, 10.0, 9.0]);
...
mod edge_case_tests {
    ...
    fn test_all_overlapping_spans() {
        let spans = vec![
            ...
            create_test_span("word2", 10.0, 0.0, 20.0, 12.0),
            create_test_span("word3", 20.0, 0.0, 20.0, 12.0),
        ];
    ...
    fn test_mostly_overlapping_with_outlier() {
        let spans = vec![
            create_test_span("word1", 0.0, 0.0, 12.0, 12.0),
            create_test_span("word2", 10.0, 0.0, 12.0, 12.0),
            create_test_span("word3", 8.0, 0.0, 12.0, 12.0),
            create_test_span("word4", 30.0, 0.0, 12.0, 12.0),
        ];
...
mod backward_compatibility_tests {
    ...
    fn test_adaptive_enabled_by_default() {
        ...
        // Phase 8: Adaptive is now enabled by default ~keep
        assert!(config.use_adaptive_threshold);
...
mod performance_tests {
    ...
    fn test_multiline_document_performance() {
        let gaps_per_line = vec![vec![0.2, 0.2, 0.2]; 100];
        let spans = create_multiline_document(gaps_per_line, 20.0);
