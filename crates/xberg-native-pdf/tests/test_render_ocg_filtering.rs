...
//!
//! The whole suite exercises `xberg_native_pdf::rendering`, which is gated behind the
...
fn render_raw(doc: &PdfDocument, excluded: HashSet<String>) -> (Vec<u8>, u32, u32) {
    ...
    let img = xberg_native_pdf::rendering::render_page(doc, 0, &options).expect("render");
...
fn sample_region(data: &[u8], width: u32, height: u32, x: u32, y: u32, w: u32, h: u32) -> (f32, f32, f32, f32) {
    ...
    for py in y..(y + h).min(height) {
        for px in x..(x + w).min(width) {
            ...
            if idx + 3 < data.len() {
                // tiny-skia stores premultiplied RGBA; un-premultiply for comparison ~keep
...
fn test_render_ocg_rect_visible_without_filter() {
    ...
    // At 72 DPI, 1pt = 1px. PDF y=550..750 → pixel y = 792-750..792-550 = 42..242 ~keep
...
fn test_render_unrelated_layer_filter_preserves_content() {
    ...
    // Exclude a layer that doesn't exist — should not affect rendering ~keep
...
fn build_pdf_with_clip_inside_excluded_bdc() -> Vec<u8> {
    ...
    // properly gated.
    //
    // Note: the path uses moveto/lineto and clip with the non-zero rule. ~keep
...
fn test_clip_does_not_leak_from_excluded_layer() {
    ...
    // pixel y = 792-400..792-200 = 392..592 at 72 DPI) must paint freely. ~keep
...
fn build_pdf_with_oc_annotation() -> Vec<u8> {
    ...
    // 1 Catalog, 2 Pages, 3 Page, 4 Annot, 5 AP /N stream, 6 OCG. ~keep
    offsets.push(pdf.len());
    pdf.extend_from_slice(
        b"1 0 obj\n<< /Type /Catalog /Pages 2 0 R\n\
           /OCProperties << /OCGs [6 0 R] /D << /ON [6 0 R] >> >> >>\nendobj\n\n",
...
fn test_annotation_oc_is_filtered() {
    ...
    // -> pixel y=42..242 at 72 DPI), so sampling at pixel (100,80) sees red. ~keep
...
fn test_text_advance_preserved_through_excluded_run() {
    ...
    // (i.e. clearly to the right of pixel 220). ~keep
    ...
    // after pixel 240 (where World should land after the suppressed SECRET). ~keep
    let baseline_y: u32 = 386;
    let h_strip: u32 = 18;
    ...
    // before SECRET begins. There may be background or trailing whitespace. ~keep
    ...
    // pure background. With the fix, dark glyph ink shows up here. ~keep
    let world_x: u32 = 275;
...
fn build_pdf_with_deeply_nested_bdc() -> Vec<u8> {
    ...
    // the stack, the rect must be suppressed. ~keep
...
fn test_deeply_nested_bdc_stack_unwinds_correctly() {
    ...
    // Red rect at PDF (50,550)-(250,750) → pixel x=50..250, y=42..242. ~keep
    let (r, g, b, _) = sample_region(&data, w, h, 100, 80, 50, 50);
    assert!(
        r > 250.0 && g > 250.0 && b > 250.0,
        "Inner red rect must be suppressed by /Drop scope on 12-level stack; got ({r}, {g}, {b})"
    );
    // Green rect at PDF (300,550)-(500,750) → pixel x=300..500, y=42..242. ~keep
...
fn build_pdf_multi_page() -> Vec<u8> {
    ...
    // 7 OCG-PageOne, 8 OCG-PageTwo. ~keep
    offsets.push(pdf.len());
    pdf.extend_from_slice(
        b"1 0 obj\n<< /Type /Catalog /Pages 2 0 R\n\
...
fn render_page_raw(doc: &PdfDocument, page: usize, excluded: HashSet<String>) -> (Vec<u8>, u32, u32) {
    ...
    options.format = ImageFormat::RawRgba8;
    options.excluded_layers = excluded;
    let img = xberg_native_pdf::rendering::render_page(doc, page, &options).expect("render");
    (img.data, img.width, img.height)
}
...
fn test_multi_page_per_page_filtering() {
    ...
    // Exclude PageOne only; page 1's red rect must vanish, page 2's blue rect remains. ~keep
...
// excluded — the realistic prepress use case. ~keep

#[test]
fn test_ocmd_policy_anyon() {
    // AnyOn (default): visible iff any referenced OCG is on. Hide iff all off. ~keep
    // A on, B on  -> visible
...
fn test_ocmd_policy_allon() {
    // AllOn: visible iff all on. Hide iff any off. ~keep
...
fn test_ocmd_policy_anyoff() {
    // AnyOff: visible iff any off. Hide iff all on. ~keep
    // A off, B on -> visible
...
fn test_ocmd_policy_alloff() {
    // AllOff: visible iff all off. Hide iff any on. ~keep
...
fn build_pdf_prepress_layers() -> Vec<u8> {
    ...
    // 6 OCG Dieline, 7 OCG Varnish, 8 OCG Artwork. ~keep
    offsets.push(pdf.len());
    pdf.extend_from_slice(
        b"1 0 obj\n<< /Type /Catalog /Pages 2 0 R\n\
    ...
    //   Text: "PRINT" overlaid, no OCG ~keep
    let content: &[u8] = b"\
