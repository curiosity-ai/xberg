...
fn build_form_xobject_with_matrix_pdf() -> Vec<u8> {
    ...
    // Text is placed at (200, 400) in XObject space → (100, 200) in page space ~keep
...
fn test_form_xobject_matrix_scaling_applied_to_spans() {
    ...
    // Allow some tolerance for font metrics. ~keep
    assert!(
        span.bbox.x < 150.0,
...
fn test_form_xobject_matrix_translation_applied_to_spans() {
    ...
    // Expected page-space x = 50 + 100 = 150 ~keep
    assert!(
...
fn test_form_xobject_nested_matrix_composition() {
    ...
    //   Form1 translate: (100+100, 150+50) = (200, 200) ~keep
    let mut pdf = Vec::new();
    let mut offsets: Vec<usize> = Vec::new();
    ...
    // Expected: x = 200*0.5 + 100 = 200, y = 300*0.5 + 50 = 200 ~keep
    assert!(
