...
fn rw1_full_width_title_reads_contiguously() {
    ...
    // it on top and reads the body before the metadata sidebar. ~keep
    ...
    // distinct ones so plain narrow columns and label:value forms never engage. ~keep
    let furniture = [
        "Citation 0",
        "Received 24",
        "Accepted 24",
    ...
    // The body must read before the sidebar furniture (and not be interleaved ~keep
...
// extractor keeps the digit run left-to-right (UAX #9 L2).
// =========================================================================== ~keep
#[test]
