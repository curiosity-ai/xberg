//! When deduplicating two overlapping copies of the same content,
//! prefer the flow-prose rendering over the wide-spaced positioned one.
//!
//! Scanned monographs digitised into a journal archive frequently emit
//! two copies of the same paragraph in one content stream:
//!
...
//!
//! The extractor used to emit both copies (the output had each page twice).
//! The reading-order fixes that followed dedup them correctly but consistently
...
//!
//! Current status: the synthetic assertion passes — our
//! extractor does not keep the wide-spaced positioned copy in
//! preference to the flow-prose copy. Validated against the
//! scanned-monograph fixture that motivated it: its history-of-technology
//! body prose comes through as clean flow prose without the
//! run-of-many-spaces rendering or a duplicate wide-spaced copy. The cumulative effect of running-header
//! removal, XY-cut, stroke+fill overlap handling, and content-based
...
fn two_copy_pdf() -> Vec<u8> {
    ...
    //   "In 1894 a new single-cylinder gasoline engine installed"
    //
    ...
    // the journal-archive producer ordering the report described. ~keep
    let content = "\
BT /F0 12 Tf 1 0 0 1 50 800 Tm (In 1894 a new single-cylinder gasoline engine installed) Tj ET\n\
BT /F0 12 Tf 1 0 0 1 50 800 Tm (In) Tj \
50 0 Td (1894) Tj \
50 0 Td (a) Tj \
50 0 Td (new) Tj \
50 0 Td (single-cylinder) Tj \
60 0 Td (gasoline) Tj \
60 0 Td (engine) Tj \
...
fn dedup_prefers_flow_prose_copy_over_positioned_copy() {
    ...
    assert!(
        !text.contains("    "),
        "output contains a 4+ space run, which means the positioned \
