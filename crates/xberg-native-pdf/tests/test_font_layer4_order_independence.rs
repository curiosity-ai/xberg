//! Regression test for the Layer 4 font cache (`font_name_set_cache`): it must not
...
//!
//! Reproducer confirmed against a several-hundred-page tax-form instruction
//! booklet: page 48 loses 41 non-whitespace characters when extracted after
//! pages 0–47.
...
fn two_page_same_font_key_different_cmap(char1: char, char2: char) -> Vec<u8> {
    ...
    // 4: Page 2  — font key /F1 → object 20 (different ObjectRef!) ~keep
    push(
        &mut out,
        &mut offsets,
        "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] \
    ...
    // 10: Font 1 — unique BaseFont tag + ToUnicode → object 11 ~keep
    push(
        &mut out,
        &mut offsets,
        format!(
    ...
    // 20: Font 2 — unique BaseFont tag (different suffix) + ToUnicode → object 21 ~keep
    push(
        &mut out,
        &mut offsets,
        format!(
...
fn font_layer4_cache_must_not_cross_contaminate_tounicode_cmaps() {
    let char1 = 'X'; // page 1 maps 0x41 → U+0058 ~keep
    let char2 = 'Y'; // page 2 maps 0x41 → U+0059 ~keep
...
fn font_layer4_cache_order_independence_reversed() {
    let char1 = 'P'; // page 1 maps 0x41 → U+0050 ~keep
    let char2 = 'Q'; // page 2 maps 0x41 → U+0051 ~keep
    let pdf = two_page_same_font_key_different_cmap(char1, char2);
...
/// Regression guard against the tax-form-booklet reproducer pattern:
...
fn font_layer4_nonws_count_is_order_independent() {
    let char1 = 'A'; // page 1 maps 0x41 → U+0041 ~keep
    let char2 = 'B'; // page 2 maps 0x41 → U+0042 ~keep
    let pdf = two_page_same_font_key_different_cmap(char1, char2);
    ...
    assert_eq!(
        ...
        "non-whitespace char count for page 1 must be identical whether or not \
         page 0 was extracted first (tax-form booklet pattern): \
         clean={}, after={}",
        nonws(&clean),
        nonws(&after)
    );
