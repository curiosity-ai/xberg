...
// - `off[id]`  start of object definition ~keep
...
fn build_pdf_with_page_extras(page_count: usize, extra_per_page: impl Fn(usize) -> String) -> Vec<u8> {
    ...
    // records where the xref table itself starts. ~keep
    ...
    for i in 0..page_count {
        ...
        // object w/ drawing instructions `/Contents` with content-stream object ~keep
        obj(
            &mut buf,
            &mut off,
            page_id,
    ...
    // records where THIS table itself starts (needed for the trailer). ~keep
    ...
    // giving its 10-digit zero-padded byte offset ~keep
    for offset in &off[1..] {
...
fn remove_footers_preserves_repeated_phrase_that_drifts_in_x() {
    ...
    // beyond the x tolerance (72, 152, 232, 312, 392 → spread 320pt). ~keep
    let bytes = build_pdf_with_page_extras(5, |i| {
...
fn remove_footers_preserves_body_twin_of_footer_text() {
    ...
    // [must survive], on every page. ~keep
