...
fn spot_pdf(func_obj5: Option<&str>) -> Vec<u8> {
    ...
    match func_obj5 {
        ...
        None => {
            ...
            // 8-bit samples: t=0 -> (FF FF FF) white, t=1 -> (FF 00 00) red. ~keep
...
fn type0_sampled_tint_renders_alternate_colour_not_gray() {
    ...
    // gray = 1 - tint = black. Allow generous tolerance for colour management. ~keep
    assert!(
        r > 200 && g < 80 && b < 80,
...
fn type3_stitching_tint_renders_second_slice_colour() {
    ...
    // Filling at tint 1.0 lands in the second slice -> green. ~keep
