//! Article-thread (`/Threads`) parsing (ISO 32000-1:2008 §12.4.3).
//!
...
use xberg_native_pdf::structure::parse_article_threads;
...
fn thread_right_before_left_pdf() -> Vec<u8> {
    ...
    // LEFTWORD (higher) first; the thread reads the right column (RIGHTWORD) first. ~keep
    ...
    // Bead B — left column (read second). ~keep
    obj(
        &mut buf,
        &mut off,
        8,
...
fn parses_cross_page_three_bead_thread() {
    ...
    // Bead A is the right column (llx ≈ 310); bead B is the left column (llx ≈ 40). ~keep
