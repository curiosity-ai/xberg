...
fn signed_pdf_opens_without_error() {
    let path = "tests/fixtures/regressions/signed_pdf_renders.pdf";
...
fn signed_pdf_renders_first_page_without_error() {
    use xberg_native_pdf::rendering::{self, RenderOptions};
    ...
    // PNG magic — the format must be what we asked for. ~keep
    assert!(
