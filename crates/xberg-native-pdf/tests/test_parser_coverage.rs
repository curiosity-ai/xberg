...
fn test_xref_entry_types() {
    ...
    assert_eq!(compressed.offset, 10);
    assert_eq!(compressed.generation, 3);
...
fn test_xref_table_merge() {
    ...
    table2.add_entry(1, XRefEntry::uncompressed(150, 1));
...
fn test_xref_table_trailer() {
    ...
    trailer.insert("Size".to_string(), xberg_native_pdf::object::Object::Integer(10));
...
fn test_structure_tree_tagged_pdf() {
    // Build a minimal tagged PDF with StructTreeRoot ~keep
    let mut pdf = b"%PDF-1.7\n".to_vec();
    let off1 = pdf.len();
    ...
    let doc = PdfDocument::from_bytes(pdf).unwrap();
    let tree = doc.structure_tree().unwrap();
    // May or may not find structure tree depending on parsing strictness ~keep
    let _ = tree;
}
