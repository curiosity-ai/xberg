//! Cyrillic text must decode correctly through a /Differences
...
fn cyrillic_differences_pdf() -> Vec<u8> {
    ...
    // afii10036 = U+0422 (Cyrillic capital Т) ~keep
    push(
        &mut out,
        &mut offsets,
...
fn cyrillic_differences_encoding_yields_unicode_cyrillic() {
    ...
    // interpreted as WinAnsi instead of mapped through /Differences. ~keep
    assert!(
        !text.contains('\u{00D0}'),
        "mojibake symptom: output contains Ð (U+00D0). Text: {text:?}"
    );
}
...
fn utf8_in_winansi_pdf() -> Vec<u8> {
    // "Лист" in UTF-8: 0xD0 0x9B 0xD0 0xB8 0xD1 0x81 0xD1 0x82 ~keep
