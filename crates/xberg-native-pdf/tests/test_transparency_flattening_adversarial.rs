...
// 0.5020 directly: G = 0.4980 * 255 ≈ 127. Distinguishable. ~keep
...
// different from subfunction-on-the-left. ~keep
...
fn adversarial_smask_tr_type3_inverted_encode_byte_exact() {
    ...
    //   G = (1 - 0.3725) * 255 = 0.6275 * 255 = 160.01 → byte 160 ~keep
    assert_eq!(
        (r, g, b),
        (255, 160, 160),
...
fn adversarial_smask_tr_type0_bps16_falls_to_identity_byte_exact() {
    ...
    // (255, 127, 127) — the same as no-/TR baseline. ~keep
    assert_eq!(
        (r, g, b),
        (255, 127, 127),
...
// constant. Pin the byte-exact reference. ~keep
...
fn adversarial_smask_tr_type0_single_sample_byte_exact() {
    ...
    // fully blocked → (255, 255, 255). ~keep
    assert_eq!(
        (r, g, b),
        (255, 255, 255),
...
// is a real fall-through. Pin the actual behaviour. ~keep
...
fn fixture_smask_tr_type4_div_by_zero() -> Vec<u8> {
    ...
    // per §7.10.5, often NaN or Inf in IEEE arithmetic. ~keep
...
// claim that the dispatch is on array length. ~keep
...
fn adversarial_smask_bc_n5_over_devicergb_group_falls_to_black_byte_exact() {
    ...
    // m=0 → backdrop white survives. Reference (255, 255, 255). ~keep
    assert_eq!(
        (r, g, b),
        (255, 255, 255),
...
// the only spec-coherent reading. ~keep
...
fn adversarial_rgb_overprint_paint_gamut_compresses_no_panic() {
    ...
    // pins the byte-exact composite AND the no-panic property. ~keep
