...
//!
//! The defect: a scanned Hebrew OCR PDF (simple TrueType font, one `TJ`
...
//! added earlier was supposed to prevent exactly this class of bug.
//!
...
//!    confidence-gated geometric detector — it still used the older
//!    `has_rtl && buffer.accumulated_width > 0.0` heuristic.
...
//! (Beitza) the report cited. Hebrew reads right-to-left, so the correct
...
const WORD_ONE_LOGICAL: &str = "\u{05D1}\u{05D9}\u{05E6}\u{05D4}";
const WORD_ONE_REVERSED: &str = "\u{05D4}\u{05E6}\u{05D9}\u{05D1}";
const WORD_TWO_LOGICAL: &str = "\u{05E4}\u{05E8}\u{05E7}";
const WORD_TWO_REVERSED: &str = "\u{05E7}\u{05E8}\u{05E4}";

/// Bug shape: each word's glyph codes are stored in
...
fn tj_array_invisible_logical_order_per_word_is_not_reversed() {
    ...
    eprintln!("[invisible] logical-per-word extracted: {text:?}");
...
/// `hebrew_mirrored.pdf`-style shape (regression guard): each word's
...
fn tj_array_visual_order_per_word_regression() {
    ...
    let doc = PdfDocument::from_bytes(pdf).expect("parse synthetic PDF");
    let text = doc.extract_text(0).expect("extract_text");
    eprintln!("[visual] visual-per-word extracted: {text:?}");
...
/// unconditionally after this fix. This asserts the actual,
