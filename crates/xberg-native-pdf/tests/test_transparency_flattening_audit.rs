...
//! pins the byte-exact behaviour `xberg-native-pdf` produces on the composite
//! render path (`xberg_native_pdf::rendering::render_page`). Every probe in
...
pub const HONEST_GAP_SMASK_BC_MALFORMED_ARITY: &str = "HONEST_GAP_SMASK_BC_MALFORMED_ARITY: /SMask /BC arity that disagrees \
...
// edge so AA does not contaminate the sample. ~keep
...
fn ca_uppercase_stroke_alpha_half_paints_faded_red_ring() {
    ...
    // land byte-exact at this position. ~keep
    assert_eq!(
        (r, g, b),
        (255, 127, 127),
...
// ===========================================================================
//
// xberg-native-pdf treats an image's `/SMask` stream as a luminance alpha mask
...
fn image_smask_alpha_paints_diagonal_red_over_white() {
    ...
    // (the y flip happens at the image-blit level, swapping rows). ~keep
...
fn smask_form_alpha_modulates_destination_alpha() {
    ...
    // background paint, byte-exact (255, 255, 255). ~keep
    assert_eq!(
        (r, g, b),
        (255, 255, 255),
...
fn fixture_smask_with_bc_backdrop() -> Vec<u8> {
    ...
    // luminance Y ≈ 127 → modulated alpha 127/255. ~keep
...
fn fixture_smask_with_tr_transfer() -> Vec<u8> {
    ...
    // modulation 0.25 → red over white at α=0.25 yields (255, 191, 191). ~keep
    let form_content = "0.5 g\n0 0 100 100 re\nf\n";
    let obj_5 = format!(
        "5 0 obj\n<< /Type /XObject /Subtype /Form /BBox [0 0 100 100] \
         /Resources << >> /Length {} >>\nstream\n{}\nendstream\nendobj\n",
...
fn smask_tr_transfer_squares_modulation() {
    ...
    //  = (255, 191.something, 191.something) → byte (255, 191, 191). ~keep
    assert_eq!(
        (r, g, b),
        (255, 191, 191),
...
fn smask_tr_type4_postscript_halves_modulation() {
    ...
    // and from no-/TR (255, 127, 127). ~keep
    assert_eq!(
        (r, g, b),
        (255, 191, 191),
...
fn smask_bc_devicen_5_components_evaluates_tint_transform() {
    ...
    // visible) and from n=1/3/4 device-family cases. ~keep
    assert_eq!(
        (r, g, b),
        (255, 56, 56),
...
//      the malformed-but-spec-permitted degenerate case. ~keep
...
fn fixture_smask_tr_type3_two_type2_subfunctions() -> Vec<u8> {
    ...
    // are a representation choice, not a requirement). ~keep
...
// onto the parent. xberg-native-pdf implements this correctly per
...
fn isolated_transparency_group_composites_red_over_blue() {
    ...
    // the exact reference is enforced. ~keep
    assert_eq!(
        (r, g, b),
        (128, 0, 127),
...
fn fixture_knockout_group_two_overlapping_rects() -> Vec<u8> {
    ...
    // results differ in the overlap region. ~keep
...
// helpers rather than degenerating to a direct render. ~keep
...
// matrix is in scope for a later round.) ~keep
...
fn blend_multiply_red_over_grey_yields_dark_red() {
    ...
    // B = 128·0/255 = 0. Byte-exact (128, 0, 0). ~keep
    assert_eq!(
        (r, g, b),
        (128, 0, 0),
...
// probe pins the spec-correct value and is `#[ignore]`-marked. ~keep
...
fn blend_hue_red_source_paints_red_hue_over_blue() {
    ...
    // (94, 0, 0). Byte-exact reference under the §11.3.5.3 algorithm. ~keep
    assert_eq!(
        (r, g, b),
        (94, 0, 0),
...
fn blend_saturation_grey_source_desaturates_red_to_grey() {
    ...
    // collapses to (0,0,0) then SetLum lifts to (0.30, 0.30, 0.30). ~keep
    assert_eq!(
        (r, g, b),
        (77, 77, 77),
...
fn fixture_blend_luminosity_grey_source_over_red() -> Vec<u8> {
    ...
    // source. SourceOver fallback paints the dark grey itself. ~keep
...
// branches on the overprint flags. ~keep
...
// model and surfaces here as a different overlap byte triple. ~keep
...
// well inside their respective active rects. ~keep
...
fn smask_form_honours_base_transform_at_144_dpi() {
    ...
    // With base_transform, mask α=255 here → red paint shows through. ~keep
    ...
    // With base_transform, mask α=0 here → white backdrop survives. ~keep
