...
impl InkRouter {
    ...
    pub(crate) fn route(
        ...
        // the per-channel participating list. ~keep
        ...
        if participating.is_empty() {
            // RGB sources don't route to plates at all. ~keep
        ...
        // leaves the plate untouched; overprint=false knocks it out. ~keep
...
mod tests {
    ...
    fn spot_plate_outside_cmyk_knocks_out_by_default() {
        ...
        // (paint 0.0 to erase underlying ink). ~keep
        let gs = fresh_gs();
        let plan = cmyk_plan(false, 0);
        let color = cmyk_color();
    ...
    fn spot_plate_outside_cmyk_skips_when_overprint() {
        // §11.7.4 with OP=true: unspecified plates are left untouched. ~keep
        let gs = fresh_gs();
    ...
    fn opm_one_skips_zero_components_on_cmyk() {
        ...
        // specified" → leave the matching plate alone. ~keep
        let gs = fresh_gs();
    ...
    fn opm_zero_paints_zero_components_normally() {
        ...
        // (which knocks the plate out at the painted pixels). ~keep
        let gs = fresh_gs();
    ...
    fn all_inks_paints_every_plate_at_single_tint() {
        ...
        // `selector: InkSelector::All` marker to short-circuit. ~keep
        let gs = fresh_gs();
        // Composite colour resolution may still produce gray-at-tint;
        // the router does not read `color` when selector is All/None. ~keep
        let color = ResolvedColor::Rgba {
            r: 0.6,
            g: 0.6,
            b: 0.6,
    ...
    fn all_inks_paints_even_when_overprint_enabled() {
        ...
        // exception. Same tint, every plate, OP=true. ~keep
        let gs = fresh_gs();
        let color = ResolvedColor::Rgba {
            r: 1.0,
            g: 1.0,
    ...
    fn none_inks_skips_every_plate() {
        ...
        // plate skips, regardless of overprint state. ~keep
        let gs = fresh_gs();
        let color = ResolvedColor::Rgba {
            r: 0.0,
            g: 0.0,
    ...
    fn devicen_channel_named_none_is_dropped() {
        ...
        let router = InkRouter::new();
        // "None" target falls through to other_plate_action — knock out. ~keep
