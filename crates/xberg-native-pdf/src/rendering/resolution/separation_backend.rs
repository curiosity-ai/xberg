...
impl PaintBackend for SeparationBackend {
    ...
    fn paint(&mut self, cmd: &ResolvedPaintCmd, surface: Self::Surface<'_>) -> Result<()> {
        ...
        // which are constant across plates. ~keep
        ...
        // approximation reaches the standard plates. ~keep
        ...
        // §8.6.6.3 fallback to it via the participating list it walks. ~keep
        let fallback_plan;
        let effective_plan: &super::resolved::OverprintPlan =
            if cmd.overprint.spot_source.is_some() && !device_has_spot_plate {
                // Device lacks the spot plate → use alt-CMYK approximation. ~keep
        ...
        for (plate_idx, ink) in surface.inks.iter().enumerate() {
            ...
            // router's surface in this wave. ~keep
...
// `ClipPlan::Mask` carries `Arc<Mask>` and the backend dereferences it. ~keep
...
mod tests {
    ...
    fn fill_routes_cmyk_to_matching_plates() {
        ...
        // R channel of each plate carries the per-ink tint. ~keep
        ...
        // The plate was zero before; painting zero leaves it zero. ~keep
    ...
    fn fill_skips_spot_plates_when_overprint_enabled() {
        ...
        // Cyan painted with tint 0.5 -> 128. ~keep
        assert_eq!(plates[0].data()[(5 * 16 + 5) * 4], 128);
        // Spot plate untouched -> sentinel R=200 still visible. ~keep
    ...
    fn per_channel_devicen_routes_named_plates() {
        ...
        // today this test constructs it directly). ~keep
        let path = rect_path();
        ...
        let mut backend = SeparationBackend::new();
        backend.paint(&cmd, surface).unwrap();
        // 0.75 -> 191 (round half away from zero), 0.1 -> 26. ~keep
    ...
    fn rgb_color_routes_to_no_plates() {
        ...
        // Skip for every plate, so every plate stays untouched. ~keep
        let path = rect_path();
        let cmd = ResolvedPaintCmd {
            kind: PaintKind::Path {
                path: &path,
            ...
            // OverprintResolver produces empty participating for RGB. ~keep
            overprint: OverprintPlan {
                enabled: false,
                mode: 0,
    ...
    fn opm1_zero_component_on_cmyk_skips_matching_plate() {
        ...
        // is enabled. Pre-fill Magenta with sentinel to verify. ~keep
        let path = rect_path();
        let mut cmd = cmyk_cmd(&path, 0.5, 0.0, 0.0, 0.0);
        cmd.overprint.enabled = true;
        ...
        let mut backend = SeparationBackend::new();
        backend.paint(&cmd, surface).unwrap();
        // Cyan painted at 0.5 -> 128. ~keep
        assert_eq!(plates[0].data()[(5 * 16 + 5) * 4], 128);
        // Magenta untouched under OPM=1 (zero source component). ~keep
    ...
    fn color_only_intent_paints_nothing() {
        ...
        // attempt to rasterise anything. ~keep
        let cmd = ResolvedPaintCmd {
    ...
    fn assert_backend_matches_inline(
        ...
        // Backend route: call into the real public `paint` API. ~keep
        ...
        // the same composed transform the backend would have used. ~keep
    ...
    fn all_inks_paints_every_plate_at_same_tint() {
        ...
        // selector and ignores the colour for routing. ~keep
        let path = rect_path();
        let cmd = ResolvedPaintCmd {
            kind: PaintKind::Path {
                path: &path,
    ...
    fn none_inks_paints_no_plates() {
        ...
        // at its initial zero value. ~keep
        let path = rect_path();
        let cmd = ResolvedPaintCmd {
            kind: PaintKind::Path {
                path: &path,
    ...
    fn cmyk_cyan_only_matches_fill_separation_byte_for_byte() {
        ...
        // separation_renderer::fill_separation for each. ~keep
        let path = rect_path();
    ...
    fn cmyk_mixed_fill_matches_fill_separation_byte_for_byte() {
        ...
        // must match its independent fill_separation invocation. ~keep
        let path = rect_path();
    ...
    fn cmyk_rotated_ctm_matches_fill_separation_byte_for_byte() {
        ...
        // Mirrors the wave 5 inline-path rotated-rect probe. ~keep
        let path = rect_path();
