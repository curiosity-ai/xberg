    ...
    pub(crate) fn resolve(&self, gs: &GraphicsState, side: PaintSide, color: &ResolvedColor) -> OverprintPlan {
        ...
        let participating: SmallVec<[ParticipatingChannel; 8]> = match color {
            ...
            ResolvedColor::Cmyk { c, m, y, k, .. } | ResolvedColor::IccCmyk { c, m, y, k, .. } => {
                ...
                // reads. The plate router doesn't care about RGB. ~keep
        ...
        OverprintPlan {
            ...
            // the OverprintResolver stays stateless and source-agnostic. ~keep
...
mod tests {
    ...
    fn default_state_yields_disabled_plan() {
        ...
        // marker only; backends short-circuit. ~keep
        let gs = fresh_gs();
    ...
    fn rgb_color_lists_no_participating_channels() {
        ...
        // set so per-plate backends naturally skip the intent. ~keep
        let mut gs = fresh_gs();
        gs.fill_overprint = true;
        let color = ResolvedColor::Rgba {
            r: 1.0,
