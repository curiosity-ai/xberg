...
mod tests {
    ...
    fn ink_name_equality_is_string_equality() {
        ...
        // …) are compared as their exact names. ~keep
    ...
    fn overprint_plan_disabled_is_no_op_marker() {
        ...
        // circuit and paint every channel. ~keep
    ...
    fn overprint_plan_participating_inline_capacity() {
        // 4-process DeviceCMYK stays inline. ~keep
        let plan = OverprintPlan {
            enabled: true,
            mode: 0,
    ...
    fn clip_plan_mask_is_arc_shared() {
        ...
        // single B/b operator share one composed mask without copying. ~keep
        match (&plan_a, &plan_b) {
