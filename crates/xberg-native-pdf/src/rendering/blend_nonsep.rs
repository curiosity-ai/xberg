...
pub(crate) fn compose_in_place(dest: &mut [u8], source: &[u8], mode: NonSeparableBlend) {
    ...
    for px in 0..(dest.len() / 4) {
        ...
        if src_a == 0 {
            ...
            // Co = Cb is the §11.3.4 reduction; nothing to write). ~keep
            continue;
        }

        // Read source/dest as premultiplied f32 in [0, 1], then
        // un-premultiply to straight colour for the §11.3.5.3 math. ~keep
        ...
        // §11.3.5.3 blend B(Cb, Cs) on STRAIGHT colour. ~keep
        let (br, bg, bb) = match mode {
            NonSeparableBlend::Hue => {
                // SetLum(SetSat(Cs, Sat(Cb)), Lum(Cb)) ~keep
        ...
        //   αo = αs + αb · (1 − αs)
        //   Co = ((1 − αs) · αb · Cb + αs · ((1 − αb) · Cs + αb · B)) / αo ~keep
        let inv_sa = 1.0 - sa;
        let inv_da = 1.0 - da;
        ...
        // Re-premultiply for tiny_skia storage. ~keep
...
fn clip_color(c: (f32, f32, f32)) -> (f32, f32, f32) {
    ...
    if n < 0.0 {
        // Scale toward the luminance to bring the minimum to 0. ~keep
        let denom = l - n;
        if denom.abs() > 1e-9 {
            r = l + (r - l) * l / denom;
            g = l + (g - l) * l / denom;
    ...
    if x > 1.0 {
        // Scale toward the luminance to bring the maximum to 1. ~keep
        let denom = x - l;
        if denom.abs() > 1e-9 {
            r = l + (r - l) * (1.0 - l) / denom;
            g = l + (g - l) * (1.0 - l) / denom;
...
fn set_sat(c: (f32, f32, f32), s: f32) -> (f32, f32, f32) {
    ...
    // max - min, mid is scaled proportionally, others zero. ~keep
    let (r, g, b) = c;
    // Sort channels by value, tracking original positions. ~keep
...
mod tests {
    ...
    fn luminosity_blend_grey_source_over_red_preserves_red_hue() {
        ...
        //   b = 0.286
        // Result is red-dominant (R=1.0 >> G≈0.286, B≈0.286). ~keep
    ...
    fn saturation_blend_grey_source_desaturates_dest() {
        ...
        //   = (0.30, 0.30, 0.30) → grey. ~keep
        let mut dest = [255u8, 0, 0, 255];
        let source = [128u8, 128, 128, 255];
        compose_in_place(&mut dest, &source, NonSeparableBlend::Saturation);
        // Channels should be near-equal (desaturated). ~keep
    ...
    // also the bug write-up for the per-channel arithmetic. ~keep
    ...
    fn hue_blend_partial_alpha_is_byte_exact() {
        ...
        // αs=179/255, αb=128/255 and re-premultiply → (57, 19, 179, 217). ~keep
    ...
    fn saturation_blend_partial_alpha_is_byte_exact() {
        ...
        // applied=red coincides; B_blue picks up the source-only term. ~keep
        let mut dest = PA_BACKDROP;
    ...
    fn color_blend_partial_alpha_is_byte_exact() {
        ...
        // SetLum(Cs, Lum(Cb)) form for both modes. ~keep
        let mut dest = PA_BACKDROP;
    ...
    fn luminosity_blend_partial_alpha_is_byte_exact() {
        ...
        // fix the backdrop would dominate. ~keep
        let mut dest = PA_BACKDROP;
