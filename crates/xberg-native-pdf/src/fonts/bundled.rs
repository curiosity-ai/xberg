...
//!
//! These TTFs ship inside the xberg-native-pdf binary so that markdown / DOCX /
...
mod tests {
    ...
    fn font_bytes_load() {
        ...
        // TTF files start with the 'true' or 0x00010000 sfnt version. ~keep
    ...
    fn smart_quote_in_winansi() {
        // U+2019 IS in WinAnsi at 0x92 — no fallback needed. ~keep
        assert!(!needs_unicode_font("Pearson\u{2019}s"));
    }

    #[test]
    fn greek_needs_unicode() {
        assert!(needs_unicode_font("β"));
        assert!(needs_unicode_font("σ"));
    }

    #[test]
    fn math_italic_does_not_need_unicode() {
        // Math italic chars get normalized to plain Latin/Greek; the Latin
        // ones land in WinAnsi, the Greek ones DO need the Unicode font. ~keep
        assert!(!needs_unicode_font("\u{1D465}"));
        assert!(needs_unicode_font("\u{1D6FD}"));
    }
}
