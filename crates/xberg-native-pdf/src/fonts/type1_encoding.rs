...
pub fn parse_type1_encoding(font_data: &[u8]) -> Option<HashMap<u8, char>> {
    ...
    // We search within the first portion of the file (clear text is typically <10KB). ~keep
    ...
    // to let the default StandardEncoding handling work. ~keep
    ...
    if trimmed.starts_with(b"StandardEncoding") {
        tracing::debug!("Type 1 font uses StandardEncoding (predefined)");
        return None;
    }
...
mod tests {
    ...
    fn test_parse_type1_encoding_ligatures() {
        ...
        assert_eq!(map.get(&12), Some(&'\u{FB01}'));
        assert_eq!(map.get(&14), Some(&'\u{FB03}'));
    }
    ...
    fn test_parse_type1_cmr_style_encoding() {
        ...
        assert_eq!(map.get(&11), Some(&'\u{FB00}'));
        assert_eq!(map.get(&12), Some(&'\u{FB01}'));
        assert_eq!(map.get(&13), Some(&'\u{FB02}'));
        assert_eq!(map.get(&14), Some(&'\u{FB03}'));
        assert_eq!(map.get(&15), Some(&'\u{FB04}'));
        assert_eq!(map.get(&65), Some(&'A'));
        assert_eq!(map.get(&97), Some(&'a'));
        assert_eq!(map.get(&48), Some(&'0'));
        assert_eq!(map.get(&58), Some(&':'));
        assert_eq!(map.get(&123), Some(&'\u{2013}'));
    }
}
