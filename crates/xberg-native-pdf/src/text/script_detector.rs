...
fn should_split_cjk_transition(prev: CJKScript, curr: CJKScript, language: Option<DocumentLanguage>) -> Option<bool> {
    ...
    match language {
        ...
        Some(DocumentLanguage::Chinese) | None => handle_chinese_transition(prev, curr),
    }
}
...
pub fn is_combining_mark(code: u32) -> bool {
    matches!(code, 0x3099 | 0x309A | 0xFF9E | 0xFF9F)
}
...
mod tests {
    ...
    fn test_small_katakana_detection() {
        ...
        assert!(is_small_katakana(0x30E3));
        assert!(!is_small_katakana(0x30A2));
    }
