...
pub fn is_sentence_ending_punctuation(code: u32) -> bool {
    matches!(code, 0x3002 | 0xFF01 | 0xFF1F)
}
...
pub fn is_enumeration_punctuation(code: u32) -> bool {
    matches!(code, 0x3001 | 0xFF0C | 0xFF1B | 0xFF1A)
}
...
pub fn is_other_cjk_punctuation(code: u32) -> bool {
    matches!(code, 0x3000 | 0x3003 | 0x30FB | 0xFF0E | 0xFF0D | 0xFF5E)
}
...
mod tests {
    ...
    fn test_text_density_classify_boundary_medium_high() {
        let density_2000 = TextDensity::classify(2000, 1);
        let density_2001 = TextDensity::classify(2001, 1);
    ...
    fn test_punctuation_score_with_density_low() {
        ...
        // With low density (0.6×), becomes 0.6 ~keep
    ...
    fn test_punctuation_score_with_density_high() {
        ...
        // With high density (1.4×), becomes 1.12 ~keep
    ...
    fn test_punctuation_score_enumeration_with_density_low() {
        ...
        // With low density (0.6×), becomes 0.54 ~keep
    ...
    fn test_punctuation_score_enumeration_with_density_high() {
        // Ideographic comma normally scores 0.9
        // With high density (1.4×), becomes 1.26 ~keep
    ...
    fn test_density_classify_multi_page() {
        let density_per_page_1000 = TextDensity::classify(3000, 3);
        assert_eq!(density_per_page_1000, TextDensity::Medium);

        let density_per_page_300 = TextDensity::classify(900, 3);
        assert_eq!(density_per_page_300, TextDensity::Low);

        let density_per_page_1500 = TextDensity::classify(9000, 6);
    ...
    fn test_bracket_scores_all_densities() {
        ...
        assert!((low - 0.48).abs() < 0.01); // 0.8 * 0.6 ~keep
        assert!((medium - 0.8).abs() < 0.01); // 0.8 * 1.0 ~keep
        assert!((high - 1.12).abs() < 0.01); // 0.8 * 1.4 ~keep
    }
}
