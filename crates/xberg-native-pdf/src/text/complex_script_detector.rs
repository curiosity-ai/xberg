...
pub fn handle_devanagari_boundary(prev_char: &CharacterInfo, curr_char: &CharacterInfo) -> Option<bool> {
    ...
    // boundary` Rule 3 (the dominant spurious-space direction). ~keep
...
pub fn handle_indic_boundary(prev_char: &CharacterInfo, curr_char: &CharacterInfo) -> Option<bool> {
    ...
    // geometric gap test inserts because the matra carries its own advance. ~keep
...
mod tests {
    ...
    fn test_devanagari_matra_detection() {
        assert!(is_devanagari_matra(0x093F));
        assert!(is_devanagari_matra(0x0940));
        assert!(is_devanagari_matra(0x0947));
        assert!(!is_devanagari_matra(0x0915));
    }

    #[test]
    fn test_devanagari_consonant_detection() {
        assert!(is_devanagari_consonant(0x0915));
        assert!(is_devanagari_consonant(0x0928));
        assert!(!is_devanagari_consonant(0x0905));
    }

    #[test]
    fn test_thai_tone_mark_detection() {
        assert!(is_thai_tone_mark(0x0E48));
        assert!(is_thai_tone_mark(0x0E49));
        assert!(!is_thai_tone_mark(0x0E01));
    }

    #[test]
    fn test_thai_vowel_modifier_detection() {
        assert!(is_thai_vowel_modifier(0x0E31));
        assert!(is_thai_vowel_modifier(0x0E34));
        assert!(!is_thai_vowel_modifier(0x0E01));
    }
    ...
    fn test_khmer_vowel_detection() {
        ...
        assert!(!is_khmer_vowel_inherent(0x1780));
    }
