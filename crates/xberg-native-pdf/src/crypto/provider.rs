...
//!
//! See `docs/CRYPTO_PROVIDERS.md` for the
//! end-to-end story.
//!
//! # Trait shape
//!
...
/// `src/encryption/aes.rs` / `src/encryption/rc4.rs` shape so
...
pub trait Signer: Send {
    ...
    /// existing CMS construction expects, so `signer.rs` can stay
    /// byte-equal.
...
///
/// - [`super::rust_provider::RustCryptoProvider`] — default,
///   uses `sha2`/`sha1`/`md-5`/`aes`/`rsa`/`p256`/`p384`. Permits
///   legacy.
/// - `super::aws_lc_provider::AwsLcProvider` (behind
