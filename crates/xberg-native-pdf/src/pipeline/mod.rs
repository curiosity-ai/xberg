...
impl TextPipeline {
    ...
    pub fn process(&self, spans: Vec<TextSpan>, context: ReadingOrderContext) -> Result<Vec<OrderedTextSpan>> {
        let mut ordered = if context.preserve_input_order && context.mcid_order.is_none() {
            ...
            // interleave the columns. ~keep
            tracing::debug!(
                count = spans.len(),
                "preserving caller-supplied order (reading-order strategy skipped)"
            );
            spans
                .into_iter()
                .enumerate()
            ...
            tracing::debug!(
                vertical_count = spans.iter().filter(|s| s.wmode == 1).count(),
                count = spans.len(),
                "vertical-majority page detected — routing through TategakiStrategy regardless of \
                 configured strategy"
            );
...
/// Right-to-left scripts read their words in the opposite direction
...
fn reorder_rtl_word_runs(ordered: &mut [OrderedTextSpan]) {
    ...
    let is_rtl_word = |o: &OrderedTextSpan| {
        ...
        for c in o.span.text.chars() {
            if c.is_ascii_alphabetic() {
                return false; // Latin letter → not a pure-RTL word ~keep
            }
    ...
    while i < ordered.len() {
        ...
        // Keep trailing space spans as separators outside the reversed run. ~keep
        let mut last = end;
    ...
    if changed {
        ...
        // converters (which may re-sort by reading_order) stay consistent. ~keep
...
mod rtl_reorder_tests {
    ...
    // plain-text read right-to-left, without char-flipping each word. ~keep
    #[test]
