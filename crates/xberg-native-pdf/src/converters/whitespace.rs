...
///
/// ```
/// use xberg_native_pdf::converters::whitespace::normalize_whitespace;
///
/// let input = "Line 1\n\n\n\n\n\nLine 2";
...
pub fn normalize_whitespace(text: &str) -> String {
    ...
    // Replace with exactly 3 newlines (2 blank lines) ~keep
...
///
/// ```
/// use xberg_native_pdf::converters::whitespace::remove_page_artifacts;
///
...
///
/// ```no_run
/// use xberg_native_pdf::converters::whitespace::merge_bold_markers;
///
...
///
/// ```no_run
/// use xberg_native_pdf::converters::whitespace::remove_duplicate_words;
///
...
///
/// ```
/// use xberg_native_pdf::converters::whitespace::cleanup_markdown;
///
/// let input = "Content\n\n\n\n\nPage 1\n\n\n\n\nMore content";
...
pub fn cleanup_markdown(text: &str) -> String {
    ...
    // because they are not spec-compliant and can corrupt legitimate text. ~keep
...
///
/// ```
/// use xberg_native_pdf::converters::whitespace::normalize_horizontal_whitespace;
///
...
///
/// ```
/// use xberg_native_pdf::converters::whitespace::cleanup_plain_text;
///
/// let input = "The  quick  brown  fox\n\n\n\n\njumps  over";
...
pub fn cleanup_plain_text(text: &str) -> String {
    ...
    // 3. Normalize vertical whitespace (limit excessive blank lines) ~keep
...
pub fn dehyphenate_line_breaks(text: &str) -> String {
    // Pre-size: dehyphenation only removes bytes, never adds. ~keep
    ...
    while i < bytes.len() {
        ...
        // ASCII fast path; otherwise re-decode the UTF-8 scalar at position i. ~keep
        if c.is_ascii() {
            out.push(c as char);
            i += 1;
        } else {
            // Multi-byte UTF-8: find the character boundary and push the full
            // scalar value, advancing i past its bytes. ~keep
            let rest = &text[i..];
...
mod tests {
    ...
    fn dehyphenate_does_not_merge_proper_noun_fragment() {
        // Uppercase start on next line: likely a new sentence / proper noun. ~keep
