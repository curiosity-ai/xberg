...
//! shall not enforce them. xberg-native-pdf surfaces them so callers who want
...
mod tests {
    ...
    fn from_p_flag_kreuzberg_password_protected_shape() {
        ...
        // — that's bits 3, 4, 5, 6 all cleared. ~keep
        let mut p: i32 = -1;
        p &= !(1 << 2);
        p &= !(1 << 3);
        p &= !(1 << 4);
        ...
        assert!(!perms.copy);
        assert!(!perms.annotate);
        // rev-3+ bits left untouched ~keep
