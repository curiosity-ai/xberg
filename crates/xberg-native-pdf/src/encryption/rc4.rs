...
/// [`crate::crypto::RustCryptoProvider`] this succeeds; under a
/// FIPS-validated provider (or any other non-legacy provider)
...
/// (`Backend`), and the policy-level `AlgorithmNotPermitted`. Only the
/// last one carries a remediation hint pointing at the active
/// provider.
///
