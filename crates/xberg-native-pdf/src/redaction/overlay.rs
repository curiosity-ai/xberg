//! Redaction overlay content-stream generation — guarantee that an
//! opaque mark is the *only* thing drawn where content was
//! removed.
//!
...
pub fn region_overlay_ops(region: &RedactionRegion, opts: &RedactionOptions) -> Vec<u8> {
    ...
    let clamp = |c: f32| -> f32 { if c.is_nan() { 0.0 } else { c.clamp(0.0, 1.0) } };
    ...
    if let Some(qd) = region.quad {
        // Polygon path over the four QuadPoints corners. ~keep
