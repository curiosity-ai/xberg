//! Standalone document sanitization — the catalog-scrub
//! decision layer (`sanitize_document`).
//!
...
//! referenced object graph and enforces the no-residual-objects guarantee.
//!
...
/// Per-category counts of what the catalog scrub removed — surfaced
/// so callers can assert the scrub did real work.
...
pub struct CatalogScrub {
    ...
    /// hard-excludes it from the output (a secret must not survive
...
/// per the enabled [`RedactionOptions`] toggles:
///
...
pub fn sanitize_catalog(
    ...
    if opts.scrub_metadata
        && let Some(o) = out.remove("Metadata")
    {
    ...
    if (want_js || want_ef) && out.contains_key("Names") {
        ...
        if let Some(mut names) = as_owned_dict(&names_obj, &resolve, &mut roots) {
            if want_js && let Some(o) = names.remove("JavaScript") {
                collect_ref_ids(&o, &mut roots);
                counts.javascript += 1;
            }
            if want_ef && let Some(o) = names.remove("EmbeddedFiles") {
                collect_ref_ids(&o, &mut roots);
...
mod tests {
    ...
    fn scrubs_metadata_and_actions_and_records_roots() {
        ...
        // every removed reference is recorded as an orphan root ~keep
        assert!(scrub.removed_roots.contains(&10));
        assert!(scrub.removed_roots.contains(&11));
        assert!(scrub.removed_roots.contains(&12));
        assert_eq!(scrub.counts.metadata_streams, 1);
        // OpenAction + AA == 2 javascript carriers ~keep
    ...
    fn scrubs_names_javascript_and_embedded_files_via_resolver() {
        ...
        names.insert("Dests".to_string(), r(23));
        ...
        // the old indirect /Names container + the removed subtrees are roots ~keep
        ...
        // OpenAction + AA + Names/JavaScript == 3 ~keep
