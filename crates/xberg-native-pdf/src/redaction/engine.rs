...
//! — the parse → prune → re-serialize → overlay pipeline that
...
//!    A composite/Type0/unknown font ⇒ **hard refusal**
//!    (`Err`), never a silent pass-through.
//! 3. `serialize` re-serializes survivors (binary-safe strings).
//! 4. `overlay` appends one opaque block per region *after* the pruned
//!    content so the redacted area is visibly marked and the
...
//! `DocumentEditor` step; residual-object destruction is
...
use super::text_engine::{FontMetrics, redact_text_stream};
...
/// the redactor's geometry cannot diverge from extraction (DRY). An unknown
/// font, or any composite/Type0 font, is
...
impl FontMetrics for FontInfoMetrics {
    ...
    fn is_simple(&self, font: &str) -> bool {
        ...
        // path here. Type0/CID and unknown ⇒ refuse (fail closed). ~keep
        match self.fonts.get(font) {
    ...
    fn decode(&self, font: &str, s: &[u8]) -> Vec<(u32, Vec<u8>)> {
        if self.is_identity_h(font) {
            ...
            // before it reaches here, so emit only whole 2-byte codes. ~keep
            return s
                .as_chunks::<2>()
                .0
                .iter()
    ...
    fn is_word_space(&self, font: &str, code: u32) -> bool {
        ...
        // it. Simple fonts keep the single-byte-32 rule. ~keep
    ...
    fn redaction_safe_show(&self, font: &str, bytes: &[u8]) -> bool {
        ...
        // refuse rather than risk mis-aligning the prune (fail closed). ~keep
...
///   a silent under-redaction (fail closed).
...
pub fn redact_content_stream(
    ...
    // CTM in between. Same fix shape qpdf uses for the identical hazard. ~keep
...
mod tests {
    ...
    /// Oracle (simplest form): the secret bytes must not appear
    ...
    fn secret_fully_in_region_is_removed_and_overlaid() {
        ...
        // An opaque overlay block is present: the overlay emits
        // `q\n<r> <g> <b> rg\n<x> <y> <w> <h> re\nf\nQ\n`. ~keep
