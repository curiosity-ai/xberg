...
//! model free of graphics-state concerns is the SRP boundary for destructive redaction.
//!
//! Safety rule: over-redaction is acceptable,
...
/// Padding applied proportionally to region height:
...
pub struct RedactionRegion {
    ...
    /// `None` mirrors an absent `/IC` entry (Table 192).
...
impl RedactionRegion {
    ...
    /// edge-padded region counts as inside. The
    ...
    /// winding-order independent, edge-inclusive. The classifier pads
...
/// this type only stores and queries them.
...
mod tests {
    ...
    fn from_quad_derives_axis_aligned_envelope() {
        // A 45°-ish rotated quad; envelope is the min/max of corners. ~keep
    ...
    fn effective_padding_floor_dominates_for_short_region() {
        // height 10 → proportional = 0.2 < 0.5 floor. ~keep
    ...
    fn effective_padding_proportional_dominates_for_tall_region() {
        // height 100 → proportional = 2.0 > 0.5 floor. ~keep
    ...
    fn effective_padding_custom_floor_is_honored() {
        ...
        // proportional = 0.1; custom floor 3.0 dominates. ~keep
    ...
    fn intersects_rect_conservative_overlap() {
        ...
        // (conservative — defeats the anti-aliased-sliver recovery). ~keep
    ...
    fn quad_contains_point_is_precise_and_winding_independent() {
        ...
        // A bbox corner (0,0) is inside the envelope but OUTSIDE the diamond. ~keep
        assert_eq!(r.quad_contains_point(&Point::new(2.0, 2.0)), Some(false));
        // Edge midpoint is inclusive. ~keep
    ...
    fn full_page_region_intersects_any_mark() {
        // A region == MediaBox intersects every mark on the page. ~keep
    ...
    fn non_finite_inputs_do_not_panic() {
        // Adversarial: NaN/Inf coords must not panic (no-panic property). ~keep
