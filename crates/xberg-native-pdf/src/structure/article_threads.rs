...
pub fn parse_article_threads(doc: &PdfDocument) -> Vec<ArticleThread> {
    ...
    // Map page ObjectRef -> 0-based page index for resolving each bead's /P. ~keep
...
fn parse_one_thread(
    ...
    // First bead is required (/F). Walk /N until we loop back to it. ~keep
    ...
    while let Some(bead_ref) = cur {
        if !seen.insert(bead_ref) || beads.len() >= MAX_BEADS_PER_THREAD {
            break; // circular wrap (normal terminator) or runaway chain ~keep
        }
        ...
        // Advance to /N (next bead). Absent /N ends the chain. ~keep
