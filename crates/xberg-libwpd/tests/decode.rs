...
//!
//! Decoding of real documents into a structured [`xberg_libwpd::WpdDocument`]
//! is covered in `ground_truth.rs` against the `test_documents/` submodule.
//! No WordPerfect binaries live in this crate.

#[test]
fn empty_input_is_not_supported() {
    assert!(!xberg_libwpd::is_supported(&[]));
    assert!(xberg_libwpd::extract_document(&[]).is_err());
}
...
fn random_bytes_do_not_crash() {
    ...
    assert!(xberg_libwpd::extract_document(&junk).is_err());
}
...
fn wordperfect_magic_but_truncated_body_fails_gracefully() {
    ...
    assert!(xberg_libwpd::extract_document(&buf).is_err());
}

/// `extract_document`/`is_supported` construct a fresh `DocumentBuilder` per
/// call and share no mutable state across the FFI boundary, so concurrent
/// callers must not crash or corrupt each other's output. Runs on the same
/// deterministic junk buffer from every thread so a data race would show up
/// as a spurious `Ok` or a panic, not just noise.
#[test]
