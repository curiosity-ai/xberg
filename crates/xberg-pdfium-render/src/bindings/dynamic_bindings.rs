...
impl DynamicPdfiumBindings {
    ...
    /// Returns `true` if a symbol with the given name is present in `library`.
    ///
    /// Used only to probe for symbol presence: the returned `Symbol` is never called, so the
    /// probe type does not need to match the function's real signature.
    fn symbol_exists(library: &Library, function: &str) -> bool {
        match CString::new(function) {
            Ok(c_function) => unsafe { library.get::<*const ()>(c_function.as_bytes_with_nul()).is_ok() },
            Err(_) => false,
        }
    }

    pub fn new(library: Library) -> Result<Self, PdfiumError> {
        // Every FPDF_* symbol this crate binds against. Probed up front so that, if the
        // library is missing several of them, the caller sees every missing symbol in one
        // error instead of only the first one `Self::bind()` happens to hit: the eager
        // struct-literal construction below uses `?` on each `Self::bind()` call and would
        // otherwise abort at the first failure, hiding the rest.
        const REQUIRED_SYMBOLS: [&str; 446] = [
        ...
        let missing_symbols: Vec<&str> = REQUIRED_SYMBOLS
            .into_iter()
            .filter(|name| !Self::symbol_exists(&library, name))
            .collect();

        if !missing_symbols.is_empty() {
            return Err(PdfiumError::LoadLibraryFunctionNameError(format!(
                "{} of {} required Pdfium symbol(s) not found in library: {}",
                missing_symbols.len(),
