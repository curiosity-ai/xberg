...
| Path traversal in ZIP archives | `resolve_container_entry()` in `extractors::security` resolves an entry name against its container-relative base by walking `std::path::Component`s rather than searching for a substring. It is boundary-relative, not `..`-phobic: an in-bounds `..` that leaves and returns without crossing the container root resolves normally (`../media/image1.png` is the ordinary OPC shape for a package-root image), and only a `..` that pops past the root is rejected, along with NUL bytes and drive-letter/UNC prefixes. Percent-decoding is the caller's job and must happen first — decoding after resolution would let a decoded `../` slip past a check that already ran. Filesystem confinement is a separate concern handled by `core::path_resolver`. |
