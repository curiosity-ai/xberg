...
// alef:hash:0b905df4fa45622b15fe7112c93d40e60a3239f6f0b48565466df16bc97508b8
// alef:handle-abi:1
...
pub fn extract(input: []const u8, config: []const u8) HeuristicsError![]u8 {
    ...
    if (input_handle == 0) return _error_with_message(HeuristicsError);
    defer c.xberg_extract_input_free(input_handle);
    const config_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{config}, 0);
    defer std.heap.c_allocator.free(config_z);
    const config_handle = c.xberg_extraction_config_from_json(config_z);
    if (config_handle == 0) return _error_with_message(HeuristicsError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_extract(input_handle, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(HeuristicsError);
    }
    return blk: {
        if (_result == 0) return error.UnknownFfiError;
        const value = _result;
...
pub fn extract_batch(inputs: []const u8, config: []const u8) HeuristicsError![]u8 {
...
pub fn map_url(uri: []const u8, config: []const u8) HeuristicsError![]u8 {
